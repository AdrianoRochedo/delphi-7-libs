...
uses
  ...
  StdCtrls, ComCtrls, ExtCtrls, RARegAuto;
...
uses RAUtils, IParser, RAI2, fDebug, fRegClasses;
...
const
  ...
  begin
    ...
    // Add('{ function ' + Name + ParamStr + ': ' + Typ + ' }');
    Add('{ ' + Decl + ' }');
		Add('procedure ' + 'RAI2_' + Name + GetArgs + ';');
		Add('begin');
		Add('  Value := ' + Result2V(Name + ConvertParams) + ';');
		Add('end;');
		Adapter.Add('    AddFun(' + UnitNameStr + '''' + Name + ''', ' +
			ClassName + 'RAI2_' + Name + ', ' + IntToStr(Params.Count) + ', ' + ParamTypStr + ', ' + ResVar + ');');
  end;    { AddFun }
			...
			if PropWrite then
				...
				begin
					...
					Adapter.Add('    AddSet(' + ClassName + ', ''' + Name + ''', ' +
						ClassName + '_' + 'Write_' + Name + ', 1, [' + ResVar + ']);');
				end
				else
				begin
					Add('');
					...
					Adapter.Add('    AddISet(' + ClassName + ', ''' + Name + ''', ' +
						ClassName + '_' + 'Write_' + Name + ', 1, [1], ' + ResVar + ');');
          if IndexDefault then
  ...
  begin
    ...
    begin
      ...
      Adapter.Add('    AddConst(' + UnitNameStr + '''' + En + ''', ' + En + ');');
      if Token = ')' then Break;
    end;    { while }
    Adapter.Add('');
  end;    { ReadEnum }
...
begin
  ...
  try
    ...
    begin
      ...
      Add('');
      Add(' { adapter unit - converts RAI2 calls to delphi calls }');
      Add(' { automatically generated by Pas2RAI2 }');
      Add('');
      ...
      Add('   - sets with more when 32 elements can''t be used in RAI2   }');
      Add('');
      Add('interface');
      Add('');
      Add('uses RAI2;');
      Add('');
      Add('  procedure RegisterRAI2Adapter(RAI2Adapter: TRAI2Adapter);');
      Add('');
      Add('implementation');
      Add('');
      Add('uses ' + ChangeFileExt(ExtractFileName(eSource.Text), '') + ';');
      Add('');
      Adapter.Add('procedure RegisterRAI2Adapter(RAI2Adapter: TRAI2Adapter);');
      Adapter.Add('begin');
      Adapter.Add('  with RAI2Adapter do');
      Adapter.Add('  begin');
    end;
    Roll := 0;
    NextToken;
    ...
    begin
      ...
      Adapter.Add('end;    { RegisterRAI2Adapter }');
      Add('');
      Add('');
