...
uses RAUtils, Consts, RACnst, iMTracer;
...
begin
  ...
  for i := P downto 1 do
    // add by patofan
    if (S[i] in Separators ) and ( StrByteType( pchar(s) , i-1 ) <> mbTrailByte )  then
    // ending add by patofan
      break;
  Beg := i + 1;
...
begin
  ...
  if P > 1 then
    // add by patofan
    if (S[P] in Separators ) and ( StrByteType( pchar(s) , p-1 ) <> mbTrailByte ) then
    // ending add by patofan
  ...
  while iBeg >= 1 do
    // add by patofan
    if ( S[iBeg] in Separators ) and ( StrByteType( pchar(s) , iBeg-1 ) <> mbTrailByte )  then
    // ending add by patofan
      break
    else
      dec(iBeg);
  ...
  while iEnd <= Length(S) do
    if ( S[iEnd] in Separators ) and ( StrByteType( pchar(s) , iEnd-1 ) <> mbTrailByte )
