...
{ history (R&A Library versions):
  ...
  1.61:
   - bug fixed: variable types are not always kept the same when you
...
type
 ...
 { function descriptor }
  ...
  protected
    constructor Create(AOwner: TRAI2Expression; AInstance: TObject;
...
constructor TRAI2Event.Create(AOwner: TRAI2Expression; AInstance: TObject;
...
begin
  ...
  begin
    ...
    begin
      Event := RAI2EventDesc.EventClass.Create(AOwner, AObject, UnitName, FunName);
...
begin
  ...
  else if (Args.Obj is TRAI2SrcUnit) then
    FunDesc := FAdapter.FindFunDesc((Args.Obj as TRAI2SrcUnit).Identifer,
      Identifer)
  else
    FunDesc := nil;
