...
type
  ...
  TMoveEvent = procedure(Sender : TRADBMove; Table : TTable; var Action : TMoveAction) of object;

  TRADBMove = class(TComponent)
  private
    SDatabase : string;
    DDatabase : string;
    STable : TTable;
...
constructor TRADBMove.Create(AOwner : TComponent);
begin
...
procedure TRADBMove.SetTables(Value : TStrings);
begin
  FTables.Assign(Value);
  CompatTables;
end;

procedure TRADBMove.CompatTables;
var
  i: integer;
begin
 { make compatible with previous version of TRADBMove }
  for i := 0 to FTables.Count - 1 do    { Iterate }
    if FTables[i] <> '' then
      FTables[i] := Trim(SubStr(FTables[i], 0, '='));
end;    { CompatTables }

procedure TRADBMove.SetReferences(Value : TStrings);
begin
  FReferences.Assign(Value);
end;

procedure TRADBMove.SetMappings(Value : TStrings);
begin
  FMappings.Assign(Value);
end;

function TRADBMove.Map(const TableName, FieldName: string): string;
begin
  if FieldName = '' then
  begin
...
procedure TRADBMove.CreateTmpTable;
begin
  with RTable do
  begin
...
procedure TRADBMove.CompileReferences;
var
  i, j : integer;
  S : string;
