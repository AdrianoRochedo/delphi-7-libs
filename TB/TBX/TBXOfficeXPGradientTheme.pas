...
//
// Further modified by Aaron Chan to provide for gradient selection of items
...
// 28.03.2003 - Aaron Chan aerodynamica@idz.net
//              Gradient selection of items
//              Had to modify alot of stuff to support SpinEdits
//              Changed draghandle back to normal one.
//              Theme can probably be called OfficeXPGradient
...
type
  ...
  TTBXOfficeXPGradientTheme = class(TTBXTheme)
  private
...
{ TTBXOfficeXPGradientTheme }

function TTBXOfficeXPGradientTheme.GetBooleanMetrics(Index: Integer): Boolean;
begin
  case Index of
...
function TTBXOfficeXPGradientTheme.GetIntegerMetrics(Index: Integer): Integer;
const
  DEFAULT = -1;
begin
  case Index of
...
function TTBXOfficeXPGradientTheme.GetViewColor(AViewType: Integer): TColor;
begin
  Result := clBtnFace;
...
function TTBXOfficeXPGradientTheme.GetBtnColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
function TTBXOfficeXPGradientTheme.GetPartColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
function TTBXOfficeXPGradientTheme.GetItemColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

function TTBXOfficeXPGradientTheme.GetItemImageBackground(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetBtnColor(ItemInfo, ipBody);
  if Result = clNone then result := GetViewColor(ItemInfo.ViewType);
end;

procedure TTBXOfficeXPGradientTheme.GetViewBorder(ViewType: Integer; out Border: TPoint);
const
...
procedure TTBXOfficeXPGradientTheme.GetMargins(MarginID: Integer; out Margins: TTBXMargins);
begin
  with Margins do
    case MarginID of
      MID_TOOLBARITEM:
...
procedure TTBXOfficeXPGradientTheme.PaintBackgnd(Canvas: TCanvas; const ADockRect, ARect, AClipRect: TRect;
...
begin
  ...
  begin
    ...
    begin
      ...
      or (ADockRect.Right <> ARect.Right) or (ADockRect.Bottom <> ARect.Bottom) then
...
procedure TTBXOfficeXPGradientTheme.PaintCaption(Canvas: TCanvas;
...
procedure TTBXOfficeXPGradientTheme.PaintCheckMark(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  X, Y: Integer;
begin
...
procedure TTBXOfficeXPGradientTheme.PaintChevron(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
const
...
procedure TTBXOfficeXPGradientTheme.PaintEditButton(Canvas: TCanvas; const ARect: TRect;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FrameRectEx(Canvas, BR, GetBtnColor(ItemInfo, ipFrame), True);

          //FillRectEx(Canvas, BR, GetBtnColor(ItemInfo, ipBody));
         end //PaintButton(Canvas, BR, ItemInfo)else if (ItemInfo.ViewType and VT_TOOLBAR) = VT_TOOLBAR then
      ...
      begin
        ...
        begin
         // FillGradient(Canvas.Handle, R, gradCol2, GetBtnColor(ItemInfo, ipBody), TGTopBottom);
          FrameRectEx(Canvas, BR, GetBtnColor(ItemInfo, ipFrame), True);
         end //PaintButton(Canvas, BR, ItemInfo)
        else if (ItemInfo.ViewType and VT_TOOLBAR) = VT_TOOLBAR then
        begin
...
procedure TTBXOfficeXPGradientTheme.PaintEditFrame(Canvas: TCanvas;
...
begin
  ...
  with EditInfo do
  begin
    if LeftBtnWidth > 0 then Inc(R.Left, LeftBtnWidth - 2);
...
procedure TTBXOfficeXPGradientTheme.PaintDropDownArrow(Canvas: TCanvas;
...
procedure TTBXOfficeXPGradientTheme.PaintButton(Canvas: TCanvas; const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  R: TRect;
begin
  with ItemInfo, Canvas do
  begin
    ...
    begin
      ...
      if (Selected) and ((ViewType and VT_TOOLBAR) = VT_TOOLBAR) then begin
        if IsVertical then
          FillGradient(Canvas.Handle, R, GetBtnColor(ItemInfo, ipBody), gradCol2, TGLeftRight)
        else
          FillGradient(Canvas.Handle, R, GetBtnColor(ItemInfo, ipBody), gradCol2, TGTopBottom);
      end else
      begin
        if (HoverKind <> hkNone) and ((ViewType and VT_TOOLBAR) = VT_TOOLBAR) then
        begin
          if Pushed then
            FillGradient(Canvas.Handle, R, gradCol2, GetBtnColor(ItemInfo, ipBody), TGTopBottom)
          else
...
procedure TTBXOfficeXPGradientTheme.PaintFloatingBorder(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOfficeXPGradientTheme.PaintFrame(Canvas: TCanvas; const ARect: TRect;
...
begin
  ...
  with ItemInfo do
    if Enabled and (HoverKind <> hkNone) then
      FillGradient(Canvas.Handle, R, GetPartColor(ItemInfo, ipBody), gradCol2, TGLeftRight);
  //FillRectEx(Canvas, R, GetPartColor(ItemInfo, ipBody));
  FrameRectEx(Canvas, R, GetPartColor(ItemInfo, ipFrame), True);
end;

function TTBXOfficeXPGradientTheme.GetImageOffset(Canvas: TCanvas;
...
procedure TTBXOfficeXPGradientTheme.PaintImage(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXOfficeXPGradientTheme.PaintMDIButton(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXOfficeXPGradientTheme.PaintMenuItemFrame(Canvas: TCanvas;
  const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  R: TRect;
begin
  ...
  begin
    ...
    //FillRect(R);
    FillGradient(Handle, R, gradCol1, gradCol2, tGLeftRight);
    Inc(R.Left);
...
procedure TTBXOfficeXPGradientTheme.PaintMenuItem(Canvas: TCanvas; var ARect: TRect;
...
procedure TTBXOfficeXPGradientTheme.PaintPopupNCArea(Canvas: TCanvas; R: TRect; const PopupInfo: TTBXPopupInfo);
var
  PR: TRect;
begin
  with Canvas do
...
procedure TTBXOfficeXPGradientTheme.PaintSeparator(Canvas: TCanvas; ARect: TRect;
...
begin
  ...
  begin
    ...
    begin
      ...
      Left:= Left - 1;
      Right:= Right + 2;
      DrawLineEx(Canvas, Left, Top, Right, Top, PopupSeparatorColor);
...
procedure TTBXOfficeXPGradientTheme.PaintToolbarNCArea(Canvas: TCanvas; R: TRect;
...
begin
  with Canvas do
  begin
    if ((ToolbarInfo.ViewType and TVT_NORMALTOOLBAR) = TVT_NORMALTOOLBAR) then begin
      with R do begin
        if (R.Right > R.Bottom) then begin
          FillGradient(Canvas.Handle, R, gradCol1, gradCol2, TGTopBottom);
          R2:= R;
          R2.Left:= R2.Right - 1;
          FillGradient(Canvas.Handle, R2, GradBL, gradCol1, TGTopBottom);
    ...
    begin
      FillGradient(Handle, R, gradCol2, gradCol1, TGLeftRight);    
      InflateRect(R, -2, -2);
    end;
...
procedure TTBXOfficeXPGradientTheme.PaintDockPanelNCArea(Canvas: TCanvas; R: TRect; const DockPanelInfo: TTBXDockPanelInfo);
...
(*procedure TTBXOfficeXPGradientTheme.SetupColorCache;
var
  DC: HDC;
...
begin
  ...
  // gradColors rmk
  gradCol1:= clBtnFace;
  gradCol2:= RGB(245, 244, 242);
  gradHandle1:= clBtnFace;
  gradHandle2:= clGray;
  gradHandle3:= clWhite;
  gradBL:= RGB(219, 216, 209); //clBtnShadow;
  ...
  begin
    ...
    PopupSeparatorColor := RGB(166,166,166); //clBtnShadow;
    ToolbarSeparatorColor := RGB(166,166,166); //clBtnShadow;
  end
 else
  begin
    { View/Window Colors }
...
function TTBXOfficeXPGradientTheme.GetPopupShadowType: Integer;
begin
  Result := PST_OFFICEXP;
end;

constructor TTBXOfficeXPGradientTheme.Create(const AName: string);
begin
  inherited;
...
destructor TTBXOfficeXPGradientTheme.Destroy;
begin
...
procedure TTBXOfficeXPGradientTheme.GetViewMargins(ViewType: Integer;
...
procedure TTBXOfficeXPGradientTheme.PaintPageScrollButton(Canvas: TCanvas;
...
procedure TTBXOfficeXPGradientTheme.PaintGlyph(Canvas: TCanvas; R: TRect; Kind, State: Integer);
var
  X, Y: Integer;
...
procedure TTBXOfficeXPGradientTheme.PaintStatusBar(Canvas: TCanvas; R: TRect; Part: Integer);
var
  D, Sz, I: Integer;
...
procedure TTBXOfficeXPGradientTheme.TBXSysCommand(var Message: TMessage);
begin
  if Message.WParam = TSC_VIEWCHANGE then SetupColorCache;
end;

initialization
  RegisterTBXTheme('OfficeXPGradient', TTBXOfficeXPGradientTheme);
end.
