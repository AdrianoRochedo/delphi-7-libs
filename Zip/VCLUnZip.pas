...
Const
  kpThisVersion = 218; {added this constant 3/1/98 for version 2.03}
...
type
 ...
 protected
   ...
   Key: DecryptKey;
    ...
    function UpdCRC(Octet: Byte; Crc: {U_LONG}DWORD) : {U_LONG}DWORD;
   ...
   procedure SetFilesList( Value: TStrings );
    ...
    function GetCrc(Index: Integer): {U_LONG}DWORD;
    function GetCompressedSize(Index: Integer): LongInt;
    function GetUnCompressedSize(Index: Integer): LongInt;
    function GetExternalFileAttributes(Index: Integer): {U_LONG}DWORD;
  ...
  public
    ...
    property Crc[Index: Integer]: {U_LONG}DWORD read GetCrc;
    ...
    property ExternalFileAttributes[Index: Integer]: {U_LONG}DWORD read GetExternalFileAttributes;
  ...
  published
    ...
    property ReplaceReadOnly: Boolean read FReplaceReadOnly write FReplaceReadOnly default False;
     { Event Properties }
  ...
  if not (csDesigning In ComponentState) then
   begin
     ...
     If not DelphiIsRunning then
      begin
        {$IFDEF NO_RES}
        MessageBox(0, 'This unregistered verion of VCLZip will only run while the Delphi IDE is running','Warning',mb_OK );
        {$ELSE}
...
function TVCLUnZip.GetCrc(Index: Integer): {U_LONG}DWORD;
var
 finfo: TZipHeaderInfo;
begin
  If (Index > -1) and (Index < Count) then
...
function TVCLUnZip.GetExternalFileAttributes(Index: Integer): {U_LONG}DWORD;
var
 finfo: TZipHeaderInfo;
begin
  If (Index > -1) and (Index < Count) then
...
begin
  ...
  begin
     ...
     StrPCopy( MsgArray, 'Please insert disk ' + IntToStr(NextDisk) + ' of the multi-disk set.' );
      {$ELSE}
     StrPCopy( MsgArray, LoadStr(IDS_INSERTDISK) + IntToStr(NextDisk) + LoadStr(IDS_OFMULTISET) );
      {$ENDIF}
...
{ $Id: VCLUnZip.pas,v 1.15 1999-04-10 10:19:38-04 kp Exp kp $ }
