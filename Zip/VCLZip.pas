        ...
        If Assigned(FOnFilePercentDone) then
         begin
           While (AmtToCopy > 0) do
            begin
              progressPartition := min(OUTBUFSIZ, AmtToCopy);
...
begin
  ...
  if (OptionsLen > 0) then
   begin
     SFXFile := TLFNFileStream.CreateFile(SFXStub, fmOpenRead);  { Get the Stub }
     SFXStubFile := TLFNFileStream.CreateFile(TemporaryPath+'tmpstub.exe', fmCreate);
...
{ $Id: VCLZip.pas,v 1.13 1999-04-10 10:18:29-04 kp Exp kp $ }
