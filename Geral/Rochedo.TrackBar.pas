...
type
  ...
  TdrTrackBar = class(TCustomControl)
  private
    FRTLFactor: integer;
    FBackColor: TColor;
    FTickColor: TColor;
...
{$R Rochedo.TrackBar.res}

implementation

procedure Register;
begin
  RegisterComponents('AD-2', [TdrTrackBar]);
end;

constructor TdrTrackBar.Create(AOwner: TComponent);
begin
...
begin
  ...
  if FOrientation = trHorizontal then
    ...
    FPosition := FAbsPos * (FMax - FMin) / FAbsLength + FMin;
    //FAbsPos := (FAbsLength / (FMax - FMin)) * (FPosition - FMin);
    SetPosition(FPosition);

    FThumbRect.Left:= 4;
    FThumbRect.Right:= FThumbRect.Left + FThumbLength;
    FThumbRect.Top:= Round(FAbsPos);
...
procedure TdrTrackBar.MouseEnter(var msg: TMessage);
begin
  Paint;
end;

procedure TdrTrackBar.MouseLeave(var msg: TMessage);
begin
...
procedure TdrTrackBar.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if not Enabled then Exit;
...
procedure TdrTrackBar.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if not Enabled then Exit;
...
procedure TdrTrackBar.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  if not Enabled then Exit;
...
procedure TdrTrackBar.Resize;
begin
  Paint;
end;

procedure TdrTrackBar.CMEnabledChanged(var msg: TMessage);
begin
  inherited;
  Paint;
end;

procedure TdrTrackBar.SetColors(Index: Integer; Value: TColor);
begin
  case Index of
    0:  FBackColor:= Value;
    1:  FTickColor:= Value;
...
procedure TdrTrackBar.SetPosition(Value: real);
begin
...
procedure TdrTrackBar.SetMin(Value: real);
begin
  FMin:= Value;
...
procedure TdrTrackBar.SetMax(Value: real);
begin
  FMax:= Value;
...
{
procedure TdrTrackBar.SetFrequency(Value: real);
begin
  FFrequency:= Value;
  Paint;
end;
}
procedure TdrTrackBar.SetThumbLength(Value: Integer);
begin
  FThumbLength:= Value;
  Paint;
end;

procedure TdrTrackBar.SetThumbWidth(Value: Integer);
begin
  FThumbWidth:= Value;
  Paint;
end;
...
{
procedure TdrTrackBar.SetTickStyle(Value: TTickStyle);
begin
  FTickStyle:= Value;
  Paint;
end;

procedure TdrTrackBar.SetTickMarks(Value: TTickMark);
begin
  FTickMarks:= Value;
  Paint;
end;
...
procedure TdrTrackBar.SetOrientation(Value: TTrackBarOrientation);
begin
  FOrientation:= Value;
  Paint;
end;

procedure TdrTrackBar.SetSmoothCorners(Value: Boolean);
begin
  FSmoothCorners:= Value;
  Paint;
end;

procedure TdrTrackBar.SetColorScheme(Value: TColorScheme);
begin
  FColorScheme:= Value;
...
function TdrTrackBar.GetColorScheme: TStringList;
const
  t = '                ';
begin
...
procedure TdrTrackBar.GradientFillRect(Canvas: TCanvas; Rect: TRect;
...
function TdrTrackBar.PointInRect(X, Y: Integer; R: TRect): Boolean;
begin
