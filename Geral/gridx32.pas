...
{                                                       }
{        Copyright (c) 1998  Rochedo                    }
{                                                       }
{*******************************************************}

{
 Versões
    1.1 (21/12/2000) - Modificado o Método TdrStringAlignGrid.DrawCell
    1.2 (22/12/2000) - Criada a propriedade AsFloat[Col, Row]
                     - Método TdrStringAlignGrid.CellToReal melhorado
                     - Bug fix no método TdrStringAlignGrid.Clear
    1.3 (14/07/2001) - Bug fix em AddBitmap                 
}
unit gridx32;
...
type
  ...
  TdrInplaceEdit = class(TCustomMaskEdit)
  private
    FGrid: TdrCustomGrid;
    FClickTime: Longint;
    procedure InternalMove(const Loc: TRect; Redraw: Boolean);
    procedure SetGrid(Value: TdrCustomGrid);
  ...
  protected
    ...
    property  Grid: TdrCustomGrid read FGrid;
  public
  ...
  { TdrCustomGrid is an abstract base class that can be used to implement
    ...
    Protected members, for implementors of TdrCustomGrid descendents
      DesignOptionBoost
  ...
  TCellChangeEvent = procedure (Sender: TObject; oldCol, oldRow, newCol, newRow: Longint) of object;

  TdrCustomGrid = class(TCustomControl)
  private
    FAnchor: TGridCoord;
  ...
  protected
    ...
    VirtualView: Boolean;
    function GetInplaceRect(r: TRect): TRect; virtual; {Rochedo}
    ...
    function CreateEditor: TdrInplaceEdit; virtual;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    procedure KeyPress(var Key: Char); override;
    ...
    property FocusTextColor: TColor read FFocusTextColor write SetFocusTextColor default clHighLightText;
...
//    property InplaceEditor: TdrInplaceEdit read FInplaceEdit;
  ...
  TdrDrawGrid = class(TdrCustomGrid)
  private
  ...
  { TdrStringGrid adds to TdrDrawGrid the ability to save a string and associated
  ...
  TdrStringGridStrings = class(TStrings)
  private
    FGrid: TdrStringGrid;
    FIndex: Integer;
  ...
  public
    constructor Create(AGrid: TdrStringGrid; AIndex: Longint);
  ...
  TdrStringGrid = class(TdrDrawGrid)
  private
    FData: Pointer;
    FRows: Pointer;
    FCols: Pointer;
    ...
    function EnsureColRow(Index: Integer; IsCol: Boolean): TdrStringGridStrings;
...
{Rochedo}
  ...
  TChooseFontEvent = procedure (aCol, aRow: Integer; aState: TGridDrawState) of Object;

  TdrStringAlignGrid = class(TdrStringGrid)
  private
    FAlign             : TMyAlign;
    FAlignCol          : TList;
    FFAlignCol         : TList;
    FObjectsAreBitmaps : Boolean;
    FOnChooseFont      : TChooseFontEvent;
    ...
    procedure Initialize;
    procedure CopiaDados(p: pChar; Separador: Char);
    procedure ColaDados(p: pChar; Separador: Char);
    function GetFloat(aCol, aRow: Integer): Real;
    procedure setFloat(aCol, aRow: Integer; const Value: Real);
    procedure SetBoolean(c,r: Integer; Value: Boolean);
  ...
  public
    ...
    procedure DrawBitmap(C,L: Integer; G: TGraphic);
    procedure AddBitmap(C,L: Integer; G: TGraphic);
    procedure ClearObjects;
    procedure Clear(Fixed: Boolean);
    ...
    procedure LoadFromXLS(const FileName: String);
    {$endif}

    procedure Copy(Separador: Char = #9);
    procedure Paste(Separador: Char = #9);
    function  isCell(SubStr: String; var ACol, ARow: LongInt): Boolean;
    function  CellToReal(ACol, ARow: Integer; var ConvError: Boolean): Real;
    procedure SetFloatF(aCol, aRow: Integer; const Value: Double; Digits: Integer);
    ...
    property ObjectsAreBitmaps: Boolean read FObjectsAreBitmaps write FObjectsAreBitmaps;
    property AsFloat[aCol, aRow: Integer]: Real read GetFloat write setFloat;
    property AsInteger[aCol, aRow: Integer]: Integer read GetInteger write setInteger;
    property AsBoolean[aCol, aRow: Integer]: Boolean write SetBoolean;

  published
    property Alignment: TMyAlign read GetAlign write SetAlign default alLeft;
    property OnChooseFont : TChooseFontEvent read FOnChooseFont write FOnChooseFont;
  end;

  TdrCheckBoxGrid = Class(TdrStringAlignGrid)
  Private
    CheckedList : TList;
    procedure WMLButtonDown(var Message: TWMLButtonDown); message WM_LBUTTONDOWN;
    Function  GetChecked(col, row: Longint): Boolean;
    Procedure SetChecked(col, row: Longint; Value: Boolean);
  Protected
    function GetInplaceRect(r: TRect): TRect; override;
  ...
  Public
    ...
    procedure RemoveCols(ColIndex, CCount : LongInt); override;
    procedure RemoveRows(RowIndex, RCount : LongInt); override;
    procedure InsertRows(RowIndex, RCount: LongInt);  override;
    procedure InsertCols(ColIndex, CCount: LongInt);  override;

    property Checked[Index: Longint; Index: Longint]: Boolean
             Read  GetChecked
             Write SetChecked;
  End;
...
implementation
uses Math, Consts, RTLConsts, Dialogs, extctrls,
     {$ifdef Excel}
     drPlanilha,
     {$endif}
     clipBrd;
...
constructor TdrInplaceEdit.Create(AOwner: TComponent);
begin
...
procedure TdrInplaceEdit.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.Style := Params.Style or ES_MULTILINE;
end;

procedure TdrInplaceEdit.SetGrid(Value: TdrCustomGrid);
begin
  FGrid := Value;
end;

procedure TdrInplaceEdit.CMShowingChanged(var Message: TMessage);
begin
  { Ignore showing using the Visible property }
end;

procedure TdrInplaceEdit.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  inherited;
...
procedure TdrInplaceEdit.WMPaste(var Message);
begin
  if not EditCanModify then Exit;
  inherited
end;

procedure TdrInplaceEdit.WMClear(var Message);
begin
  if not EditCanModify then Exit;
  inherited;
end;

procedure TdrInplaceEdit.WMCut(var Message);
begin
  if not EditCanModify then Exit;
  inherited;
end;

procedure TdrInplaceEdit.DblClick;
begin
  Grid.DblClick;
end;

function TdrInplaceEdit.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer;
  MousePos: TPoint): Boolean;
begin
  Result := Grid.DoMouseWheel(Shift, WheelDelta, MousePos);
end;

function TdrInplaceEdit.EditCanModify: Boolean;
begin
  Result := Grid.CanEditModify;
end;

procedure TdrInplaceEdit.KeyDown(var Key: Word; Shift: TShiftState);
...
procedure TdrInplaceEdit.KeyPress(var Key: Char);
var
...
procedure TdrInplaceEdit.KeyUp(var Key: Word; Shift: TShiftState);
begin
  Grid.KeyUp(Key, Shift);
end;

procedure TdrInplaceEdit.WndProc(var Message: TMessage);
begin
  case Message.Msg of
    WM_SETFOCUS:
      begin
...
procedure TdrInplaceEdit.Deselect;
begin
  SendMessage(Handle, EM_SETSEL, $7FFFFFFF, Longint($FFFFFFFF));
end;

procedure TdrInplaceEdit.Invalidate;
var
  Cur: TRect;
begin
...
function TdrInplaceEdit.PosEqual(const Rect: TRect): Boolean;
var
  Cur: TRect;
begin
...
procedure TdrInplaceEdit.InternalMove(const Loc: TRect; Redraw: Boolean);
begin
...
procedure TdrInplaceEdit.BoundsChanged;
var
  R: TRect;
begin
...
procedure TdrInplaceEdit.UpdateLoc(const Loc: TRect);
begin
  InternalMove(Loc, False);
end;

function TdrInplaceEdit.Visible: Boolean;
begin
  Result := IsWindowVisible(Handle);
end;

procedure TdrInplaceEdit.Move(const Loc: TRect);
begin
  InternalMove(Loc, True);
end;

procedure TdrInplaceEdit.SetFocus;
begin
  if IsWindowVisible(Handle) then
    Windows.SetFocus(Handle);
end;

procedure TdrInplaceEdit.UpdateContents;
begin
  Text := '';
...
constructor TdrCustomGrid.Create(AOwner: TComponent);
const
...
destructor TdrCustomGrid.Destroy;
begin
  FInplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
...
procedure TdrCustomGrid.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function TdrCustomGrid.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure TdrCustomGrid.DoExit;
begin
  inherited DoExit;
  if not (goAlwaysShowEditor in Options) then HideEditor;
end;

function TdrCustomGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function TdrCustomGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;

function TdrCustomGrid.CanEditShow: Boolean;
begin
...
function TdrCustomGrid.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function TdrCustomGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function TdrCustomGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure TdrCustomGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function TdrCustomGrid.GetEditLimit: Integer;
begin
  Result := 0;
end;

procedure TdrCustomGrid.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure TdrCustomGrid.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure TdrCustomGrid.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure TdrCustomGrid.InvalidateEditor;
begin
  FInplaceCol := -1;
  FInplaceRow := -1;
  UpdateEdit;
...
procedure TdrCustomGrid.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TdrCustomGrid.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TdrCustomGrid.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TdrCustomGrid.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TdrCustomGrid.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TdrCustomGrid(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TdrCustomGrid(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure TdrCustomGrid.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure TdrCustomGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure TdrCustomGrid.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure TdrCustomGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function TdrCustomGrid.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure TdrCustomGrid.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function TdrCustomGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure TdrCustomGrid.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function TdrCustomGrid.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TdrCustomGrid.TopLeftChanged;
begin
...
function TdrCustomGrid.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TdrCustomGrid.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure TdrCustomGrid.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
procedure TdrCustomGrid.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function TdrCustomGrid.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TdrCustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
...
procedure TdrCustomGrid.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure TdrCustomGrid.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure TdrCustomGrid.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
procedure TdrCustomGrid.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure TdrCustomGrid.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure TdrCustomGrid.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure TdrCustomGrid.GridRectToScreenRect(GridRect: TGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TdrCustomGrid.Initialize;
begin
...
procedure TdrCustomGrid.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TdrCustomGrid.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TdrCustomGrid.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TdrCustomGrid.InvalidateGrid;
begin
  Invalidate;
end;

procedure TdrCustomGrid.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure TdrCustomGrid.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure TdrCustomGrid.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure TdrCustomGrid.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure TdrCustomGrid.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
begin
  ...
  begin
    if Assigned(FOnCellChange) then FOnCellChange(Self, FCurrent.X, FCurrent.Y, ACol, ARow); // dri
    OldSel := Selection;
    OldCurrent := FCurrent;
    FCurrent.X := ACol;
    FCurrent.Y := ARow;
...
procedure TdrCustomGrid.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure TdrCustomGrid.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TdrCustomGrid.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TdrCustomGrid.SelectionMoved(const OldSel: TGridRect);
var
...
procedure TdrCustomGrid.ScrollDataInfo(DX, DY: Integer;
...
procedure TdrCustomGrid.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TdrCustomGrid.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure TdrCustomGrid.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure TdrCustomGrid.UpdateScrollRange;
var
...
function TdrCustomGrid.GetInplaceRect(r: TRect): TRect;  {Rochedo}
begin
  Result := r;
end;

function TdrCustomGrid.CreateEditor: TdrInplaceEdit;
begin
  Result := TdrInplaceEdit.Create(Self);
end;

procedure TdrCustomGrid.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
  begin
...
procedure TdrCustomGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TdrCustomGrid.KeyPress(var Key: Char);
begin
...
procedure TdrCustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
procedure TdrCustomGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure TdrCustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
...
procedure TdrCustomGrid.MoveAndScroll(Mouse, CellHit: Integer;
...
function TdrCustomGrid.GetColWidths(Index: Longint): Integer;
begin
...
function TdrCustomGrid.GetRowHeights(Index: Longint): Integer;
begin
...
function TdrCustomGrid.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TdrCustomGrid.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TdrCustomGrid.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function TdrCustomGrid.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function TdrCustomGrid.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TdrCustomGrid.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TdrCustomGrid.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TdrCustomGrid.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure TdrCustomGrid.SetColCount(Value: Longint);
begin
...
procedure TdrCustomGrid.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure TdrCustomGrid.SetDefaultColWidth(Value: Integer);
begin
...
procedure TdrCustomGrid.SetDefaultRowHeight(Value: Integer);
begin
...
procedure TdrCustomGrid.SetFixedColor(Value: TColor);
begin
...
procedure TdrCustomGrid.SetFocusColor(Value: TColor);
begin
...
procedure TdrCustomGrid.SetFocusTextColor(Value: TColor);
begin
...
procedure TdrCustomGrid.SetFixedCols(Value: Integer);
begin
...
procedure TdrCustomGrid.SetFixedRows(Value: Integer);
begin
...
procedure TdrCustomGrid.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure TdrCustomGrid.SetGridLineWidth(Value: Integer);
begin
...
procedure TdrCustomGrid.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure TdrCustomGrid.SetOptions(Value: TGridOptions);
begin
...
procedure TdrCustomGrid.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure TdrCustomGrid.SetRowCount(Value: Longint);
begin
...
procedure TdrCustomGrid.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure TdrCustomGrid.SetScrollBars(Value: TScrollStyle);
begin
...
procedure TdrCustomGrid.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure TdrCustomGrid.SetTabStops(Index: Longint; Value: Boolean);
begin
...
procedure TdrCustomGrid.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure TdrCustomGrid.HideEdit;
begin
  if FInplaceEdit <> nil then
    try
      UpdateText;
...
procedure TdrCustomGrid.UpdateEdit;
...
begin
  ...
  begin
    ...
    r := GetInplaceRect(CellRect(Col, Row));  {Rochedo}
    if CanEditShow then FInplaceEdit.Move(r);
  end;
end;

procedure TdrCustomGrid.UpdateText;
begin
  if (FInplaceCol <> -1) and (FInplaceRow <> -1) then
    SetEditText(FInplaceCol, FInplaceRow, FInplaceEdit.Text);
end;

procedure TdrCustomGrid.WMChar(var Msg: TWMChar);
begin
...
procedure TdrCustomGrid.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure TdrCustomGrid.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure TdrCustomGrid.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure TdrCustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  if FInplaceEdit <> nil then FInplaceEdit.FClickTime := GetMessageTime;
end;

procedure TdrCustomGrid.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure TdrCustomGrid.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TdrCustomGrid.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
...
procedure TdrCustomGrid.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
...
procedure TdrCustomGrid.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TdrCustomGrid.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TdrCustomGrid.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure TdrCustomGrid.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure TdrCustomGrid.CMCancelMode(var Msg: TMessage);
begin
...
procedure TdrCustomGrid.CMFontChanged(var Message: TMessage);
begin
  if FInplaceEdit <> nil then FInplaceEdit.Font := Font;
  inherited;
end;

procedure TdrCustomGrid.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure TdrCustomGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure TdrCustomGrid.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure TdrCustomGrid.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure TdrCustomGrid.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure TdrCustomGrid.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TdrCustomGrid.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TdrCustomGrid.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure TdrCustomGrid.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure TdrCustomGrid.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function TdrCustomGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TdrCustomGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TdrCustomGrid.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TdrCustomGrid.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure TdrCustomGrid.CMShowingChanged(var Message: TMessage);
begin
  inherited;
...
function TdrDrawGrid.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := inherited CellRect(ACol, ARow);
end;

procedure TdrDrawGrid.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TdrDrawGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnColumnMoved) then FOnColumnMoved(Self, FromIndex, ToIndex);
end;

function TdrDrawGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditMask) then FOnGetEditMask(Self, ACol, ARow, Result);
end;

function TdrDrawGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TdrDrawGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
  if Assigned(FOnRowMoved) then FOnRowMoved(Self, FromIndex, ToIndex);
end;

function TdrDrawGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
  if Assigned(FOnSelectCell) then FOnSelectCell(Self, ACol, ARow, Result);
end;

procedure TdrDrawGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  if Assigned(FOnSetEditText) then FOnSetEditText(Self, ACol, ARow, Value);
end;

procedure TdrDrawGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
procedure TdrDrawGrid.TopLeftChanged;
begin
...
{ Sparse array classes for TdrStringGrid }
...
procedure TdrStringGridStrings.Assign(Source: TPersistent);
var
  I, Max: Integer;
begin
...
procedure TdrStringGridStrings.CalcXY(Index: Integer; var X, Y: Integer);
begin
  if FIndex = 0 then
  begin
    X := -1; Y := -1;
...
function TdrStringGridStrings.Add(const S: string): Integer;
var
  I: Integer;
begin
...
procedure TdrStringGridStrings.Clear;
var
...
procedure TdrStringGridStrings.Delete(Index: Integer);
begin
  InvalidOp(sInvalidStringGridOp);
end;

function TdrStringGridStrings.Get(Index: Integer): string;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
function TdrStringGridStrings.GetCount: Integer;
begin
...
function TdrStringGridStrings.GetObject(Index: Integer): TObject;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TdrStringGridStrings.Insert(Index: Integer; const S: string);
begin
  InvalidOp(sInvalidStringGridOp);
end;

procedure TdrStringGridStrings.Put(Index: Integer; const S: string);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TdrStringGridStrings.PutObject(Index: Integer; AObject: TObject);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TdrStringGridStrings.SetUpdateState(Updating: Boolean);
begin
...
constructor TdrStringGrid.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Initialize;
end;

destructor TdrStringGrid.Destroy;
...
procedure TdrStringGrid.ColumnMoved(FromIndex, ToIndex: Longint);
...
procedure TdrStringGrid.RowMoved(FromIndex, ToIndex: Longint);
begin
...
function TdrStringGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := Cells[ACol, ARow];
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TdrStringGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  DisableEditUpdate;
  try
...
procedure TdrStringGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TGridDrawState);
begin
...
procedure TdrStringGrid.DisableEditUpdate;
begin
  Inc(FEditUpdate);
end;

procedure TdrStringGrid.EnableEditUpdate;
begin
  Dec(FEditUpdate);
end;

procedure TdrStringGrid.Initialize;
var
  quantum: TSPAQuantum;
begin
  if FCols = nil then
...
procedure TdrStringGrid.SetUpdateState(Updating: Boolean);
begin
  FUpdating := Updating;
...
procedure TdrStringGrid.Update(ACol, ARow: Integer);
begin
...
function  TdrStringGrid.EnsureColRow(Index: Integer; IsCol: Boolean):
  TdrStringGridStrings;
var
  RCIndex: Integer;
  PList: ^TSparseList;
begin
  ...
  Result := TdrStringGridStrings(PList^[Index]);
  if Result = nil then
  begin
    if IsCol then RCIndex := -Index - 1 else RCIndex := Index + 1;
    Result := TdrStringGridStrings.Create(Self, RCIndex);
    PList^[Index] := Result;
  end;
end;

function  TdrStringGrid.EnsureDataRow(ARow: Integer): Pointer;
var
  quantum: TSPAQuantum;
begin
...
function TdrStringGrid.GetCells(ACol, ARow: Integer): string;
var
...
function TdrStringGrid.GetCols(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, True);
end;

function TdrStringGrid.GetObjects(ACol, ARow: Integer): TObject;
var
  ssl: TStringSparseList;
begin
  ssl := TStringSparseList(TSparseList(FData)[ARow]);
...
function TdrStringGrid.GetRows(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, False);
end;

procedure TdrStringGrid.SetCells(ACol, ARow: Integer; const Value: string);
begin
  TdrStringGridStrings(EnsureDataRow(ARow))[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  Update(ACol, ARow);
end;

procedure TdrStringGrid.SetCols(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, True).Assign(Value);
end;

procedure TdrStringGrid.SetObjects(ACol, ARow: Integer; Value: TObject);
begin
  TdrStringGridStrings(EnsureDataRow(ARow)).Objects[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  Update(ACol, ARow);
end;

procedure TdrStringGrid.SetRows(Index: Integer; Value: TStrings);
begin
...
constructor TdrStringAlignGrid.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Initialize;
end;

destructor TdrStringAlignGrid.Destroy;
var i: integer;
begin
   ...
   if FObjectsAreBitmaps then ClearObjects;
   inherited Destroy;
end;

procedure TdrStringAlignGrid.ClearObjects;
var i,j: Integer;
Begin
  For i := 0 to ColCount-1 do
    For j := 0 to RowCount-1 do
      If Objects[i,j] is TGraphic Then
         Begin
         TGraphic(Objects[i,j]).Free;
         Objects[i,j] := Nil;
         End;
End;

procedure TdrStringAlignGrid.Clear(Fixed: Boolean);
var i, j, C, R: Integer;
Begin
  if FObjectsAreBitmaps then ClearObjects;
  If Fixed Then
  ...
  For i := C to ColCount-1 do
    For j := R to RowCount-1 do
      Cells[i, j] := '';
End;

procedure TdrStringAlignGrid.Paint;
var i,j: Integer;
Begin
  Inherited Paint;
  if FObjectsAreBitmaps then
  For i := 0 to ColCount-1 do
    For j := 0 to RowCount-1 do
      If Objects[i,j] is TGraphic Then
         DrawBitmap(i,j, TGraphic(Objects[i,j]));
End;

procedure TdrStringAlignGrid.DrawBitmap(C, L: Integer; G: TGraphic);
var R: TRect;
Begin
  R := CellRect(C, L);
...
procedure TdrStringAlignGrid.AddBitmap(C,L: Integer; G: TGraphic);
var B: Graphics.TBitmap;
Begin
  {Cells[C,L] := ' ';}

   If (Objects[C,L] is Graphics.TBitmap) Then
      Begin
      Graphics.TBitmap(Objects[C,L]).Assign(G);
      End
   Else
      Begin
...
procedure TdrStringAlignGrid.Initialize;
begin
   FAlignCol := TList.Create;
   FFAlignCol := TList.Create;
   FAlign := alLeft;
   FObjectsAreBitmaps := True; 
end;

function TdrStringAlignGrid.GetAlignCol(ACol:Integer):TMyAlign;
var v:^tmyalign;
begin
...
procedure TdrStringAlignGrid.SetAlignCol(ACol:integer; const Value: TMyAlign);
var i:integer;
    v:^tmyalign;
begin
   if ACol+1 > FAlignCol.Count then
...
function TdrStringAlignGrid.GetFixAlignCol(ACol:Integer):TMyAlign;
var v:^tmyalign;
begin
...
procedure TdrStringAlignGrid.SetFixAlignCol(ACol:integer; const Value: TMyAlign);
var i:integer;
    v:^tmyalign;
begin
   if ACol+1 > FFAlignCol.Count then
...
function TdrStringAlignGrid.GetAlign: TMyAlign;
begin
   Result := FAlign;
end;

procedure TdrStringAlignGrid.SetAlign(const Value: TMyAlign);
begin
   FAlign := Value;
   Invalidate;
end;
...
procedure TdrStringAlignGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TGridDrawState);
...
begin
  if Assigned(FOnChooseFont) then
     FOnChooseFont(aCol, aRow, aState); // OBS: Utilize Canvas.Font
{
  if DefaultDrawing then
     DrawCellText
  Else
...
{TdrStringAlignGrid.RemoveRows}
procedure TdrStringAlignGrid.RemoveRows(RowIndex, RCount : LongInt);
var
  i: LongInt;
begin
  for i := RowIndex to (RowCount - 1 - RCount) do
    Rows[i] := Rows[i + RCount];
  RowCount := RowCount - RCount;
end;

{TdrStringAlignGrid.InsertRows}
procedure TdrStringAlignGrid.InsertRows(RowIndex, RCount : LongInt);
var
  i: LongInt;
begin
  RowCount := RowCount + RCount;
...
{TdrStringAlignGrid.RemoveCols}
procedure TdrStringAlignGrid.RemoveCols(ColIndex, CCount : LongInt);
var i: LongInt;
begin
  {for i := ColIndex to ColCount - 1 do}
  for i := ColIndex to (ColCount - 1 - CCount) do
    Cols[i] := Cols[i + CCount];

  ColCount := ColCount - CCount;
end;

{TdrStringAlignGrid.InsertCols}
procedure TdrStringAlignGrid.InsertCols(ColIndex, CCount : LongInt);
var
  i: LongInt;
begin
  ColCount := ColCount + CCount;
...
{TdrStringAlignGrid.isCell}
function TdrStringAlignGrid.isCell(SubStr: String; var ACol, ARow: LongInt): Boolean;
var
  i, j: LongInt;
begin
...
{TdrStringAlignGrid.SaveToFile}
procedure TdrStringAlignGrid.SaveToFile(const FileName: String);
var
  i, j: LongInt;
  ss: string;
  f: TextFile;
...
{TdrStringAlignGrid.LoadFromFile}
procedure TdrStringAlignGrid.LoadFromFile(const FileName: String);
var
  X, Y: Integer;
  ss, ss1: string;
  f: TextFile;
begin
  AssignFile(f, FileName);
  Reset(f);
  if IOResult <> 0 then raise Exception.Create('File ' + FileName + ' not found');

  Readln(f, ss);
  if ss <> '' then
  begin
    ss1 := System.Copy(ss, 1, Pos(',', ss) - 1);
    ColCount := StrToInt(ss1);
    ss1 := System.Copy(ss, Pos(',', ss) + 1, Length(ss));
  ...
  begin
    ...
    ss1 := System.Copy(ss, 1, Pos(',', ss) - 1);
    ss := System.Copy(ss, Pos(',', ss) + 1, Length(ss));
    X := StrToInt(ss1);
...
{TdrStringAlignGrid.CellToReal}
function TdrStringAlignGrid.CellToReal(ACol, ARow: Integer; var ConvError: Boolean): Real;
begin
  try
...
constructor TdrCheckBoxGrid.Create(AOwner: TComponent);
var i: Integer;
    Bits: TBits;
Begin
  inherited Create(AOwner);
  ...
  For i := FixedCols To ColCount - 1 do
    begin
    Bits := TBits.Create; Bits.Size := RowCount - FixedRows;
    CheckedList.Add(Bits);
    end;
End;

destructor TdrCheckBoxGrid.Destroy;
var i: Integer;
Begin
  For i := 0 To CheckedList.Count - 1 do TBits(CheckedList[i]).Free;
  CheckedList.Free;
  inherited Destroy;
End;

Function TdrCheckBoxGrid.GetChecked(col, row: Longint): Boolean;
Begin
  Try
    Result := TBits(CheckedList[col - FixedCols])[row - FixedRows];
  Except
    Result := False;
  End;
End;

Procedure TdrCheckBoxGrid.SetChecked(col, row: Longint; Value: Boolean);
Begin
  Try
    TBits(CheckedList[col - FixedCols])[row - FixedRows] := Value;
  Except
    {Nada}
  End;
End;

procedure TdrCheckBoxGrid.SizeChanged(OldColCount, OldRowCount: Longint);
var dc, dr, i, j: Longint;
    Bits: TBits;
Begin
  dc := OldColCount - ColCount;
  dr := OldRowCount - RowCount;

  If dc <> 0 Then
     If dc < 0 Then
        {aumentou as colunas}
        For i := 1 to abs(dc) do
           begin
           Bits := TBits.Create; Bits.Size := RowCount - FixedRows;
           CheckedList.Add(Bits)
           end
     Else
        ...
        For i := 1 to abs(dc) do
           Begin
           TBits(CheckedList.Last).Free;
           CheckedList.Delete(CheckedList.Count-1);
           End;

  If dr <> 0 Then
     For i := 0 to CheckedList.Count-1 do
        Begin
        For j := (RowCount - FixedRows) to (OldRowCount - FixedRows) do
        TBits(CheckedList[i])[j + 1] := False;
        TBits(CheckedList[i]).Size := RowCount - FixedRows;
        End;
End;

procedure TdrCheckBoxGrid.RemoveCols(ColIndex, CCount : LongInt);
var i,ii: Longint;
    x, x1: TBits;
begin
  if (ColIndex >= FixedCols) and (ColIndex < ColCount-1) Then
     for i := ColIndex to (ColCount - 1 - CCount) do
       Begin
       ii := i - FixedCols;
       x  := TBits(CheckedList[ii]);
       x1 := TBits(CheckedList[ii + CCount]);
       for ii := 0 to x.Size-1 do x[ii] := x1[ii];
       end;

  Inherited RemoveCols(ColIndex, CCount);
End;

procedure TdrCheckBoxGrid.RemoveRows(RowIndex, RCount : LongInt);
var i, ii, j: LongInt;
    x       : TBits;
begin
  if (RowIndex >= FixedRows) and (RowIndex < RowCount-1) Then
     for j := 0 to CheckedList.Count - 1 do
       begin
       x := TBits(CheckedList[j]);
       for i := RowIndex to (RowCount - 1 - RCount) do
          begin
          ii := i - FixedRows + 1;
          x[ii] := x[ii + RCount]; 
...
procedure TdrCheckBoxGrid.InsertRows(RowIndex, RCount: LongInt);
begin
  showMessage('Método InsertRows ainda não implementado para a classe TdrCheckBoxGrid');
end;

procedure TdrCheckBoxGrid.InsertCols(ColIndex, CCount: LongInt);
begin
  showMessage('Método InsertCols ainda não implementado para a classe TdrCheckBoxGrid');
end;

procedure TdrCheckBoxGrid.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var r: TRect;
    dx: Integer;
  ...
  begin
    ...
    if AlignValue in [alCenter] then
       begin
       l := ((r.Right-r.Left) - l) div 2;
       Left := r.Left + l - 1;
       end;
    ...
    Top := ((r.Bottom - r.Top) - Canvas.TextHeight(Cells[ACol, ARow])) div 2;

    ExtTextOut(Canvas.Handle, Left, r.Top + Top,  ETO_CLIPPED or
               ETO_OPAQUE, @r, Text, StrLen(Text), nil);
  end;
...
begin
  ...
  If (aCol >= FixedCols) and (aRow >= FixedRows) Then
     with Canvas do
       begin
       dx  := (aRect.Bottom - aRect.top) div 4;

       r := Rect(aRect.left + dx, aRect.top + dx,
                 aRect.left + 3*dx, aRect.Bottom - dx);

       Frame3D(Canvas, r, clRed, clMaroon, 2);

       if Checked[aCol, aRow] Then Brush.Color := clGreen else Brush.Color := clWindow;
       InflateRect(r, -1, -1);
       FillRect(r);
       end;
...
function TdrCheckBoxGrid.GetInplaceRect(r: TRect): TRect;  {Rochedo}
begin
  Result := r;
  Result.Left := Result.Left + ((r.Bottom - r.top) div 4) * 3;
  Result.Top  := Result.Top + 1;
end;

procedure TdrCheckBoxGrid.WMLButtonDown(var Message: TWMLButtonDown);
var c,r: Longint;
    rect: TRect;
Begin
  MouseToCell(Message.XPos, Message.YPos, c, r);
  If (c < FixedCols) or (r < FixedRows) Then
     Inherited
  Else
     Begin
     rect := cellRect(c, r);
     If (Message.XPos - Rect.Left) <= ((rect.Bottom - rect.top) div 4) * 3 Then
        begin
        Checked[c,r] := not Checked[c,r];
        DrawCell(c, r, Rect, [gdFixed]);
        col := c; Row := r;
        end
     Else
        Inherited;
...
procedure Register;
begin
  RegisterComponents('AD-2', [TdrStringAlignGrid, TdrCheckBoxGrid]);
end;

const
  cGridIdent = 'TStringGrid'#13;

procedure TdrStringAlignGrid.ColaDados(p: pChar; Separador: Char);

  function pegaElemento(const s: String; var n: Integer): String;

    function CharPos(const s: String; c: Char; posIni: Integer): Integer;
    begin
      Result := posIni;
  ...
  var i: Integer;
  begin
    i := CharPos(s, Separador, n);
    Result := System.copy(s, n, i-n);
    n := i+1;
  end;

var i, ii, j, jj, L, C: Integer;
    SL: TStringList;
    s : String;
    PosAtual : Integer;
begin
  ...
  try
    ...
    try
      if (SL[0]+#13) <> cGridIdent then exit;
      ii := strToInt(SL[1]);  // Linhas
      jj := strToInt(SL[2]);  // Colunas
      if SL.Count <> ii + 3 then exit;
    except
      exit;
    end;
    ...
    for i := 0 to ii-1 do
      begin
      L := Row + i;
      if L >= RowCount then exit;
      posAtual := 1;
      for j := 0 to jj-1 do
        begin
        C := Col + j;
        if C >= ColCount then Break;
        s := pegaElemento(SL[i+3], posAtual);
        Cells[C, L] := s;
        end;
      end;
  finally
...
procedure TdrStringAlignGrid.CopiaDados(p: pChar; Separador: Char);
var i,j: integer;
begin
  for i := Selection.TopLeft.Y to Selection.BottomRight.Y do
    begin
    for j := Selection.TopLeft.X to Selection.BottomRight.X do
      StrCat(p, pChar(Cells[j, i] + Separador));
    StrCat(p, #13);
    end;
end;

procedure TdrStringAlignGrid.Copy(Separador: Char);
var H: THandle;
    p: pChar;
    nc, nl, i, j: Integer;
    Tam: Integer;
begin
  ...
  try
    nc := Selection.BottomRight.X - Selection.TopLeft.X + 1;
    nl := Selection.BottomRight.Y - Selection.TopLeft.Y + 1;

    // Calcula o tamanho da área necessária para alocar os dados
    Tam := Length(cGridIdent);
    Tam := Tam + Length(intToStr(nl)) + Length(intToStr(nc)) + 2; // nLinha + nColunas + 2 CRs
    for i := Selection.TopLeft.Y to Selection.BottomRight.Y do
      begin
      for j := Selection.TopLeft.X to Selection.BottomRight.X do
        inc( Tam, Length(cells[j, i])+1 ); //Tamanho da linha + espaços
      inc(Tam); // + CR
      end;
    inc(Tam); // + #0

    H := GlobalAlloc(GMEM_MOVEABLE + GMEM_DDESHARE, Tam);
    ...
    if H <> 0 then
       ...
       if p = nil then
          ...
          try
            try
              strCopy(p, pChar(cGridIdent));
              StrCat(p, pChar(intToStr(nl) + #13));
              StrCat(p, pChar(intToStr(nc) + #13));
              CopiaDados(p, Separador);
            finally
              GlobalUnlock(H);
            end;
          except
       ...
       Raise Exception.Create('Sem memória suficiente');
...
procedure TdrStringAlignGrid.Paste(Separador: Char);
var H: THandle;
    p: pChar;
begin
  Screen.Cursor := crHourGlass;
...
function TdrStringAlignGrid.GetFloat(aCol, aRow: Integer): Real;
var Error: Boolean;
begin
  Result := CellToReal(aCol, aRow, Error);
end;

procedure TdrStringAlignGrid.SetFloatF(aCol, aRow: Integer; const Value: Double; Digits: Integer);
begin
  Cells[aCol, aRow] := Format('%*.*g', [Digits+3,Digits,Value]);
end;

procedure TdrStringAlignGrid.SetFloat(aCol, aRow: Integer; const Value: Real);
begin
  Cells[aCol, aRow] := FloatToStr(Value);
end;

procedure TdrStringAlignGrid.SetBoolean(c,r: Integer; Value: Boolean);
const
  v: array[boolean] of String = ('NÃO', 'SIM');
begin
  Cells[c,r] := v[Value];
end;


function TdrStringAlignGrid.GetInteger(aCol,aRow: Integer): Integer;
begin
  Result := StrToInt(Cells[aCol, aRow]);
end;

procedure TdrStringAlignGrid.setInteger(aCol,aRow: Integer; const Value: Integer);
begin
  Cells[aCol, aRow] := IntToStr(Value);
end;

{$ifdef Excel}
procedure TdrStringAlignGrid.LoadFromXLS(const FileName: String);
var P: TPlanilha;
    i, j: Integer;
begin
  P := TPlanilha.Create;
  try
    ...
    RowCount := Trunc(P.GetFloat(1, 1));
    ColCount := Trunc(P.GetFloat(1, 2));
    For i := 0 to RowCount-1 do
      For j := 0 to ColCount-1 do
        Cells[j, i] := P.GetText(i+2, j+1);
  finally
    P.Free;
  end;
end;

procedure TdrStringAlignGrid.SaveToXLS(const FileName: String);
var P: TPlanilha;
    i, j: Integer;
begin
  P := TPlanilha.Create;
  try
    P.Write(1, 1, RowCount);
    P.Write(1, 2, ColCount);
    For i := 0 to RowCount-1 do
      For j := 0 to ColCount-1 do
        P.Write(i+2, j+1, Cells[j, i]);
...
initialization
  CF_STRING_GRID := RegisterClipboardFormat('String Grid');
