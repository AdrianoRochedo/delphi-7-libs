...
procedure TErros_DLG.FormResize(Sender: TObject);
begin
  G.ColWidths[1] := Self.Width - G.ColWidths[0] - 15;
end;

Procedure TErros_DLG.Add(ErrorType: TErrorType; Const Msg: String);
Begin
  {Seleciona Recurso}
  Case ErrorType of
...
procedure TErros_DLG.FormShow(Sender: TObject);
begin
  G.SetFocus;
end;

procedure TErros_DLG.GDrawCell(Sender: TObject; ACol, ARow: Integer;
...
function TErros_DLG.TextHeight(const s: String; Canvas: TCanvas): Integer;
var i, ii: Integer;
begin
  ii := 1;
...
procedure TErros_DLG.btnSalvarClick(Sender: TObject);
begin
  If Save.Execute Then
     SaveToFile(Save.FileName);
end;

procedure TErros_DLG.SaveToFile(Const Name: String; ToPrinter: Boolean = False);
var SL: TStrings;
...
procedure TErros_DLG.Print;
begin
  SaveToFile('', True);
end;

procedure TErros_DLG.btnImprimirClick(Sender: TObject);
begin
  Print;
end;

procedure TErros_DLG.btnLimparClick(Sender: TObject);
begin
  Clear;
end;

procedure TErros_DLG.FormDestroy(Sender: TObject);
begin
  Clear;
end;

procedure TErros_DLG.cbTopoClick(Sender: TObject);
begin
