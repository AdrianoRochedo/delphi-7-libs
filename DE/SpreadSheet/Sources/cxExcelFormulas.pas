...
uses
  ...
  {$ELSE}
  Classes, SysUtils, QDialogs, QcxExcelAccess,
  QcxExcelConst, QcxSSUtils, QcxSSTypes, QcxSSRes;
  {$ENDIF}
...
type
  ...
  protected
    ...
    function GetArguments(ACount: Integer): string;
...
begin
  ARetCol := Shortint(ACol and $FF);
  ARetRow := ARow;
  if (ACol and $4000) <> 0 then
    ARetCol := FCol + ARetCol;
  if (ACol and $8000) <> 0 then
    ARetRow := FRow + SmallInt(ARetRow);
...
begin
  ...
  Inc(FOffset);
  ADef := FuncDefByToken(PWord(@FTokens^[FOffset])^ and $7FFF);
  FUnknownFunction := ADef = nil;
  if not FUnknownFunction then
  begin
    FStringList.Add(ADef.Name + GetArguments(I));
    Inc(FOffset, 2);
  end
end;
...
function TcxFormulaReader.GetArguments(ACount: Integer): string;
...
begin
  ...
  for I := ACount downto 1 do
    Result := PopString + ListSeparator + Result;
  if ACount > 0 then
  ...
  if Assigned(Owner) then
    ...
    begin
      if (APage < BoundSheets.Count) then
        Result := BoundSheets[XltPage(APage)]
      else
