...
uses
  ...
  {$ELSE}
  Classes, Types, QGraphics, QcxSSViewInfo, QcxSSTypes,
  QcxSSStyles, QcxSSUtils, QcxExcelConst, QcxSSPainterWrapper;
  {$ENDIF}
...
begin
  ...
  if ASelRect.Top  < 0 then
    ASelRect.Top := 0;
  if ASelRect.Right >= Length(FViewInfo.Bricks) then
    ASelRect.Right := Length(FViewInfo.Bricks) - 1;
  if ASelRect.Bottom >= Length(FViewInfo.Bricks[0]) then
