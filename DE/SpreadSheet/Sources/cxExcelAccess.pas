...
uses
  ...
  {$ELSE}
  Types, QGraphics, QDialogs, QcxSSTypes,
  QcxSSUtils, QcxExcelConst, QcxSSIntf, QcxSSRes;
  {$ENDIF}
...
type
  ...
  public
    ...
    procedure SetCellValue(APage: Word; ACol, ARow: Word; const AValue: Variant);
    procedure SetColStyle(APage, ACol, AWidth: Word; IsHidden: Boolean = False); virtual;
...
begin
  ...
  if not Supports(AOwner, IcxSpreadSheetBook, FOwner) then
    FOwner := nil;
...
var
  ...
  begin
    ...
    for I := AWords^[0] to AWords^[1] do
      FCurrentSheet.SetColRowSize(htCol, I,
         Round(AWords^[2] / 36.6 * ScaledFactor), not Boolean(AWords^[4] and $01));
  end;

  procedure ReadRowFormatInfo(AData: Pointer);
  begin                                                                     
    FCurrentSheet.SetColRowSize(htRow, PWordArray(AData)^[0],
      Round(PWordArray(AData)^[3] / 20 * 1.325 * ScaledFactor),
  ...
  procedure AddCell(ACol, ARow: Integer; const AText: string;
    AStyleIndex: Integer; IsFormula: Boolean = False);
  begin
  ...
  begin
    AddCell(PWordArray(AData)^[1], PWordArray(AData)^[0], '', PWordArray(AData)^[2]);
  end;
  ...
  begin
    CurrentSheet.SetPageDimension(Min(PWordArray(AData)^[5], 50),
        Min(PIntArray(AData)^[1], 50001));
  end;
  ...
  case VarType(AValue) of
    ...
    begin
      ...
      if AWString = '' then
        ...
        begin
          if cxTryStrToFloat(AWString, AFloatValue) then
...
begin
  with FCurrentStylePtr^.Prev^ do
    WordData[4] := (WordData[4] and not $1) or Byte(cReadOnly in AStyle^.CellState);
end;
...
begin
  ...
  begin
    SelectPage(APage);
    ANode := GetEnd(FStorage.Pages^.SheetNodesList);
    ANode := CreateXlsListNode(brcMergeCells, Length(ARects) * 8 + 2, ANode);
    ANode^.WordData[2] := Length(ARects);
    for I := 0 to High(ARects) do
    with ANode^ do
    begin
      WordData[3 + I * 4] := ARects[I].Top and $FFFF;
      WordData[4 + I * 4] := ARects[I].Bottom and $FFFF;
      WordData[5 + I * 4] := ARects[I].Left and $FF;
      WordData[6 + I * 4] := ARects[I].Right and $FF;
    end;
  end;
end;
...
const
  ...
  AStrikeOut: array[Boolean] of Word = (0, $80);
begin
  ...
  begin
    ...
    begin
      ...
      AFontNode^.WordData[3] := AItalic[fsItalic in Style] or AStrikeOut[fsStrikeOut in Style];
...
begin
  ...
  begin
    ...
    begin
      ...
      WordData[4] := ALocked[cReadOnly in AStyle^.CellState] or AHidden[False];
