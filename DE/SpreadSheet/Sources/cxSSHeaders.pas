...
uses
  Classes, SysUtils,
  {$IFDEF VCL}
  Windows, cxSSTypes, cxSSRes;
  {$ELSE}
  QcxSSTypes, QcxSSRes, QcxClasses;
  {$ENDIF}

type
  TcxSSHeaderElementState = (hsHidden, hsLocked, hsDefault, hsScaled);
  ...
  protected
    ...
    procedure SetScaledSize(AIndex, AScale, ANewScale: Integer); virtual;
...
begin
  Result := not (hsHidden in GetItem(AIndex).States);
end;
...
begin
  AItem := GetItem(AIndex);
  if (AItem.Size = Value) or (hsLocked in AItem.States) then Exit;
  TcxSSListenerAccess(Listener).OnChangeSizeColRow(OwnerSheet as TcxSSBookSheet,
                                                 Kind, AIndex, Value);
  if AItem.Size = Value then Exit;
