...
uses
  ...
  {$ELSE}
  Types, QGraphics, QForms, QControls, QcxSSData,
  QcxSSTypes, QcxSSRes, QcxControls;
  {$ENDIF}
...
implementation
uses
  {$IFDEF VCL} cxSSheet, cxSSStyles {$ELSE} QcxSSheet, QcxSSStyles {$ENDIF};
type
...
begin
  Result := scxPopupMenuMergeCells;
end;

{ TcxSplitCellsAction }
function TcxSplitCellsAction.GetActionDescription: string;
begin
  Result := scxPopupMenuSplitCells;
end;
