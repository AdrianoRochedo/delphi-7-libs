...
uses
  ...
  {$ELSE}
  QForms, Types, QControls, QStdCtrls, QGraphics, QcxGraphics, QcxControls;
  {$ENDIF}
...
begin
  ...
  {$IFNDEF VCL}
   CaretPos := TCaretPos(Point(APos.Y, APos.X))
  {$ELSE}
...
begin
  ...
  ACaretPos := CaretPos;
  {$ELSE}
  ACaretPos := Point(CaretPos.Col, CaretPos.Line);
  {$ENDIF}
  with ACaretPos do
  begin
    ...
    {$ELSE}
    CaretPos := TCaretPos(Point(Y + 1, 0));
    {$ENDIF}
  end;
end;
...
begin
  ...
  APos := CaretPos;
  {$ELSE}
  APos := Point(CaretPos.Col, CaretPos.Line);
  {$ENDIF}
  S := Lines[APos.Y];
  case Key of
    VK_TAB:
...
begin
  ...
  try
    ...
    begin
      while (StartPos <= Length(AStr)) and (AStr[StartPos] in [#13, #10]) do
        Inc(StartPos);
      I := 0;
      while not (AStr[StartPos + I] in [#13, #10]) and (I < Length(AStr)) do
        Inc(I);
      if I > 0 then
      begin
        ...
        begin
          if Result.cx < cx then
            Result.cx := cx{$IFDEF VCL} + MinSize.cx shr 1 {$ENDIF};
        end;
        Inc(StartPos, I);
      end;
    end;
