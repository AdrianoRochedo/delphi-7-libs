...
uses
  ...
  {$ELSE}
  Types, QDialogs, QcxSSHeaders, QcxSSRes,
  QcxSSStyles, QcxSSUtils, QcxSSTypes, QcxExcelConst;
  {$ENDIF}
...
uses
  ...
  {$ELSE}
  QcxSSheet, QcxSSFormulas, QcxSSHistory;
  {$ENDIF}
...
begin
  ...
  if EqualRect(Rect(-1, -1, -1, -1), ARect) then
...
begin
  ...
  begin
    ...
    begin
      ShowMessage(scxSpreadSheetMergeCellError2);
      Exit;
    end;
  end;
  FIsDeleted := True;
...
var
  ...
  var
    ...
    begin
      ...
      SetCellRec(ALeft, ALo, GetCellRec(ALeft, AHi));
      SetCellRec(ALeft, AHi, ATmp);
    end;
...
begin
  ...
  begin
    ...
    if Result^.DataType = dtFunction then
      Result^.FuncRecPtr := TcxSSBookAccess(ParentBook).FormulasCache.Clone(Result^.FuncRecPtr);
...
{  var
  ...
  begin
    Value := Trim(Value);
    Result := (Length(Value) > 1) and (Pos('=', Value) = 1) and
      (Pos('#', Value) = 0);
{    for I := 1 to Length(Value) do
    begin
      Result := Result and (Value[I] = '=');
      if (Result and (Value[I] = '=')) or not Result then Break;
    end;}
  end;
