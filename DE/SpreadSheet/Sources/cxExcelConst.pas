...
interface
uses
  SysUtils, {$IFDEF WIN32} Graphics, {$ELSE} QGraphics, {$ENDIF}
  {$IFDEF VCL} cxSSTypes, cxSSRes; {$ELSE} QcxSSTypes, QcxSSRes; {$ENDIF}
...
type
  ...
  TcxExcelFuncDefinition = packed record
    Token: Word;
    Name: string;
    Definition: TcxProc;
...
const
  ...
  EOF: packed array[0..1] of Word = (brcEOF, 0);
    ...
    Index := $A4
     ...
     begin
       ...
       try
         ...
         if not (FormatIsCurrency(S, AIndex) or
            FormatIsNumber(S, AIndex) or FormatIsDateTime(S, AIndex)) then AIndex := 0;
       finally
         Index := AIndex;
       end;
     end;
...
begin
  if (AIndex in [$A4, $A5]) then
    Result := ((Format = 'dd/mm/yy') or (Format = 'dd\.mmmm\.yy'))
  else
    Result := FormatIsDateTime(AIndex);
  if not Result then
  begin
    HasDate := ((Pos('d', Format) <> 0)) or (Pos('y', Format) <> 0);
    HasTime := (Pos('h', Format) <> 0) or ((Pos('s', Format) <> 0)) or
      (Pos('ms', Format) <> 0);
