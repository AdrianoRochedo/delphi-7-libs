...
uses
  Classes, SysUtils, {$IFDEF WINDOWS} Windows, Graphics, {$ENDIF}
  {$IFDEF VCL}
  cxGraphics, cxClasses, cxControls, cxExcelConst, cxSSTypes,
  cxSSPainterWrapper, cxSSIntf, cxSSRes;
  {$ELSE}
  Qt, QGraphics, Types, QcxGraphics, QcxControls, QcxExcelConst,
  QcxClasses, QcxSSTypes, QcxSSPainterWrapper, QcxSSIntf, QcxSSRes;
  {$ENDIF}

type
  TcxSSStyleCacheClass = class of TcxSSStyleCache;

  EStyleError = class(Exception);
...
uses
  ...
  {$ELSE}
  QcxSSheet, QcxSSData, QcxSSHeaders;
  {$ENDIF}
...
begin
  ...
  if (AStyle^.HorzAlign = haGeneral) and  (Row > 0) and (Col >= 0) then
    StyleInfo.HorzAlign := TcxSSDataStorage(Owner)[Col, Row - 1].StylePtr^.HorzAlign
  else
