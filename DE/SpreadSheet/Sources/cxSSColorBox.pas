...
uses
  Classes, SysUtils,
  {$IFDEF WINDOWS} {$IFNDEF VCL} Windows, Messages, {$ENDIF} {$ENDIF}
  {$IFDEF VCL}
  Windows, Forms, Controls, StdCtrls, Graphics, Dialogs, cxControls,
  cxGraphics, cxExcelConst, {$IFNDEF cxLib10} cxLookAndFeelPainters, {$ENDIF}
  cxSSRes, cxSSPainterWrapper, cxSSTypes;
  {$ELSE}
  Qt, Types, QControls, QStdCtrls, QGraphics, QDialogs, QForms, QcxControls,
  QcxGraphics, QcxSSRes, QcxExcelConst, QcxSSTypes, QcxSSPainterWrapper;
  {$ENDIF}
...
class procedure TcxSSExcelColorBoxPainter.Paint(ACanvas: TcxCanvasWrapper;
  ...
  begin
    ...
    begin
      ...
      while ACanvas.Canvas.TextHeight(scxColorBoxAutomatic) > AHeight do
      begin
...
//      ACanvas.Canvas.Font.Color := clBlack;
      ...
      ACanvas.DrawText(scxColorBoxAutomatic, ARect);
      ACanvas.ExcludeClipRect(ARect);
    end;
  end;
...
begin
  ...
  begin
    ...
    DrawAutomatic;
    ACanvas.FillRect(DisplayRect, fsSolid, cxBtnFaceColor, 0);
  end;
end;
...
begin
  ...
  {$IFNDEF VCL}
  H := QOpenWidgetH(GetParentForm(Self).Handle);
  if H <> nil then
  begin
    QOpenWidget_SetWFlags(H, Cardinal(WidgetFlags_WType_Modal));
    QOpenWidget_SetWState(H, Cardinal(WidgetState_WState_Modal));
...
begin
  ...
  if H <> nil then
  begin
    QOpenWidget_clearWFlags(H, Cardinal(WidgetFlags_WType_Modal));
    QOpenWidget_clearWState(H, Cardinal(WidgetState_WState_Modal));
  end;
  {$ENDIF}
  FPopupWindow.Popup;
end;
