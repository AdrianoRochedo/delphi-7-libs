...
uses
  ...
  {$ENDIF}
  {$IFDEF VCL}
    Forms, StdCtrls, Menus, Controls, Dialogs, Graphics,  cxControls, ExtCtrls,
    cxGraphics, cxExcelConst, cxExcelAccess, cxSSHeaders, cxSSData,
    cxSSTypes, {$IFNDEF cxLib10} cxLookAndFeels, {$ENDIF}
    cxSSStyles, cxSSFormulas, cxSSViewInfo, cxSSPainters, cxSSEditors,
    cxSSUtils, cxSSPainterWrapper, cxSSIntf, cxSSRes, cxSSHistory, Clipbrd;
  {$ELSE}
    Qt, Types, QForms, QControls, QStdCtrls, QMenus, QDialogs, QGraphics, QExtCtrls,
    QcxExcelAccess, QcxExcelConst, QcxControls, QcxGraphics, QcxSSHeaders, QcxSSData,
    QcxSSTypes, QcxSSStyles, QcxSSFormulas, QcxSSViewInfo, QcxSSPainters, QcxSSEditors,
    QcxSSUtils, QcxSSPainterWrapper, QcxSSIntf, QcxSSRes, QcxSSHistory, QClipbrd;
  {$ENDIF}
...
type
  ...
  private
    ...
    procedure SetColRowSize(const AKind: TcxSSHeaderType;
      const AColumn, ASize: Integer; AVisible: Boolean = True);
  ...
  protected
    function CellReadOnly(ACol, ARow: Integer): Boolean; virtual;
  ...
  protected
    ...
    function SpreadSheetDataToClipboardData(ASheet: TcxSSBookSheet;
      const ACells: TRect): string; virtual;
  ...
  public
    ...
    function HitTest(const APoint: TPoint;
      out HitTestInfo: TcxSSHitTestInfo): Boolean; {$IFDEF VCL} virtual; {$ELSE} reintroduce; overload; {$ENDIF}
  ...
  public
    ...
    property DataAsEssFormat: string read GetESSData write SetEssData;
  end;


implementation
uses
  {$IFDEF VCL} cxSSDesigner; {$ELSE} QcxSSDesigner; {$ENDIF}
...
const
  ...
  crSSSelect = {$IFDEF WINDOWS} 3200 {$ELSE} crDefault {$ENDIF};
...
begin
  ...
  if not (Owner.ActiveSheet.ReadOnly or Owner.ReadOnly) then
  begin
    ActivateEditor;
...
begin
  ...
  begin
    ...
    if AHitType = htCaption then Owner.ActivePage := AHitRes;
...
begin
  ...
  with TcxHistoryAccess((FOwner.ParentBook as TcxCustomSpreadSheetBook).History) do
    AddComplexAction(TcxComplexAction, scxChangeCellsData, FComplexAction)
end;
...
begin
  ...
  begin
    ...
    with TcxStyleAccess(Style) do
    begin
      if not IsLoading and {IsEditMode and} Style.WordBreak then
      begin
        {$IFDEF WINDOWS}
        Canvas.SelectFont(StylePtr^.FontPtr^.FontHandle);
        {$ELSE}
        Canvas.SelectFont(StylePtr^.FontPtr^);
        {$ENDIF}
        Canvas.CalculateTextExtents(Text, R, HorzTextAlign,
          VertTextAlign, WordBreak, ASettings);
        NewH := Canvas.TextHeight('Wg');
        if (Length(ASettings.TextBricks) = 1) and (Rows.DefaultSize > NewH) then
          NewH := Rows.DefaultSize;
        TcxHeaderAccess(Rows).SetScaledSize(Row, Rows[Row],
          NewH * Length(ASettings.TextBricks));
      end;
    end;
  end;
end;
...
function TcxSSCellObject.GetDisplayText: string;
  ...
  function OrdinalToFormatStr(const AString: string; AFormat: TxlsDataFormat): string;
  var
    AFValue: Double;
    ABValue: Boolean;
    ADig: Byte;
    AColor: Word;
  begin
    ADig := TcxCustomSpreadSheetBook(Owner.ParentBook).Precision;
    Result := AString;
    if AFormat <> $31 then
    begin
...
begin
  AStr := GetCellTextValue;
  with Owner[FCol, FRow].StylePtr^ do
  begin
    ...
    begin
      ...
      begin
        if cxTryStrToFloat(AStr) then
          Result := dtaRight
        else
          if cxTryStrToBool(AStr) then
            Result := dtaCenter
          else
            Result := dtaLeft;
      end
...
begin
  ...
  Owner.History.StartComplexAction(scxClearAllAction);
  try
...
begin
  Owner.DeactivateEditor;
  StartAction := Owner.History.StartComplexAction(scxClearCells);
  try
...
begin
  ...
  if IsCut then
  begin
    TcxHistoryAccess(Owner.History).AddComplexAction(TcxComplexAction, scxCutCommand, Action);
    try
...
begin
  ...
  if ACanFormat then
  begin
    History.StartComplexAction(scxChangeCellsStyle);
    try
...
begin
  ...
  APos := 1;
  TcxHistoryAccess(Owner.History).AddComplexAction(TcxComplexAction, scxPasteCommand, Action);
  Owner.BeginUpdate;
  try
    ...
    begin
      ...
      begin
        I := APos;
        while (I < Length(S)) and (not (S[I] in [#9, #13, #10])) do Inc(I);
        if I <> APos then
        ...
        AChangedCells.BottomRight := Point(ACol, ARow);
        APos := I;
      end;
    end;
  finally
...
begin
  if not Owner.ReadOnly and not ReadOnly then
  try
...
var
  Action: TcxComplexAction;
begin
  TcxHistoryAccess(Owner.History).AddComplexAction(TcxComplexAction, scxSortCellsAction, Action);
  try
    Owner.BeginUpdate;
    try
...
var
  ...
  I, J: Integer;
begin
  if CellReadOnly(DataStorage.Col, DataStorage.Row) then Exit;
  Result := True;
...
var
  AItemsCount: Integer;
const
  AIncriment: array[Boolean] of Integer = (-1, 1);
begin
  Owner.DeactivateEditor;
  AItemsCount := TcxHeaderAccess(Rows).GetCountItems(ViewInfo.TopRow,
    Abs(WheelDelta), WheelDelta < 0);
  if (ViewInfo.TopRow > 0) and (AItemsCount = 0) then
    AItemsCount := AIncriment[WheelDelta < 0];
  ViewInfo.TopRow := ViewInfo.TopRow + AItemsCount;
  Result := True;
end;

function TcxSSBookSheet.CellReadOnly(ACol, ARow: Integer): Boolean;
begin
  Result := Owner.ReadOnly or ReadOnly;
  if not Result then
  begin
    with GetCellObject(ACol, ARow) do
    try
...
begin
  ...
  if (htResize in AHitState) or (htColHeader in AHitState) or
    (htRowHeader in AHitState) then Exit;
  DoEditorActivate;
end;
...
procedure TcxSSBookSheet.KeyDown(var Key: Word; Shift: TShiftState);
  ...
  begin
    Result := (ssAlt in Shift) and (DataStorage.SelectionState = ssMultiSelect);
  end;
...
begin
  ...
  if (Key = VK_F2) and (not ((ssShift in Shift) or (ssCtrl in Shift)
    or CellReadOnly(FEditingPos.X, FEditingPos.Y))) then
  begin
    if DoEditorActivate then
    begin
      CellEditor.SetFocus;
  ...
  begin
    ...
    case Key of
      ...
      begin
        ...
        Inc(FCurrentPos.Y, ViewInfo.TopRow);
      end;
      VK_NEXT:
      begin
        FCurrentPos.Y := Row - ACorners.Top;
...
begin
  ...
  if (Byte(Key) >= $21) and not CellReadOnly(DataStorage.Col, DataStorage.Row) then
  begin
    if DoEditorActivate then
    begin
      CellEditor.Text := Key;
...
begin
  ...
  begin
    for I := 0 to Owner.PageCount - 1 do
      if (Owner.Pages[I] <> Self) and (Owner.Pages[I].DataStorage.SheetCaption = Value) then
        raise ESpreadSheetError.Create(scxSpreadSheetInvalidSheetCaption);
...
procedure TcxSSBookSheet.SetSelectionRect(const Value: TRect);
begin
...
begin
  if Source is TcxCustomSpreadSheetBook then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Page = ASheet) and EqualRect(TRect(Area), CellsRef) then
        begin
...
begin
  ...
  for I := 0 to Length(ANames) - 1 do
    with ANames[I].Definition do
      if (Page = ASheet) and EqualRect(TRect(Area), ARect) then
      begin
        Result := I;
        Break;
      end;
...
begin
  ...
  begin
    ...
    ShowMessage(Format(scxSpreadSheetInvalidFileName, [AFileName]));
end;
    ...
    begin
      try
        ...
        try
          ClearAll;
          if PageCount <> 0 then
            Pages[0].Caption := scxSheetName + IntToStr(1)
          else
            AddSheetPage;
        finally
      ...
      ShowMessage(scxSpreadSheetInvalidStreamFormat);
  finally
    FIsLoaded := False;
    FFormulasCache.IsLoading := False;
    Screen.Cursor := crDefault;
    EndUpdate;
//    ActiveSheet.SelectionRect := Rect(1,1,1,1);
    AReader.Free;
    AdjustControls;
    UpdateControl;
...
var
    ...
    if J < 255 then
      with AHeader.Data^[J] do
        if (hsHidden in States) or (not (hsDefault in States)) then
          AWriter.SetColStyle(APage, J, Size, hsHidden in States);
  ...
  begin
    ...
    begin
      ...
      case ACell^.DataType of
        dtText:
          AWriter.SetCellValue(APage, ACol, ARow, ACell^.Text);
        dtDateTime:
...
begin
  ...
  try
    ...
    AWriter.SetProtection(-1, ReadOnly);
    for I := 0 to Length(DefinedNames) - 1 do
    ...
    begin
      ...
      AWriter.SetProtection(APage, Pages[APage].ReadOnly);
...
begin
  ...
  try
    ...
    ASheetPage._AddRef;
    if APageName = '' then
      ASheetPage.DataStorage.SheetCaption := scxSheetName + IntToStr(PageCount + 1)
    else
...
function TcxCustomSpreadSheetBook.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; {$IFNDEF VCL} const {$ENDIF} MousePos: TPoint): Boolean;
begin
...
procedure TcxCustomSpreadSheetBook.DoContextPopup(
  {$IFNDEF VCL} const {$ENDIF} MousePos: TPoint; var Handled: Boolean); 
begin
...
var
  ...
  var
    AClipRgn{$IFDEF WINDOWS}, ARgn {$ENDIF}: TcxRegionHandle;
  begin
    if FCaptionBar.Visible then
    begin
      {$IFDEF WINDOWS}
      ...
      {$ELSE}
       AClipRgn := QPainter_ClipRegion(Canvas.Handle);
       if QRegion_IsEmpty(AClipRgn) or QRegion_IsNull(AClipRgn) then
    ...
    begin
      ...
      {$IFDEF WINDOWS} ARgn := EmptyHandle; {$ENDIF}
    end;
    ...
    begin
      ...
      DeleteObject(ARgn);
      {$ELSE}
      QPainter_SetClipRegion(Canvas.Handle, AClipRgn);
...
begin
  ...
  begin
    {$IFDEF WINDOWS}
    Windows.SetFocus({$IFNDEF VCL} QWidget_winId(Handle) {$ELSE} Handle {$ENDIF});
    {$ENDIF}
    FMouseDownPos := Point(X, Y);
    if FCaptionBar.Visible and PtInRect(FCaptionBar.BoundsRect, Point(X, Y)) then
    begin
...
procedure TcxCustomSpreadSheetBook.SetActivePage(const AValue: Integer);
var
  ACanSelect: Boolean;
const
...
begin
  if Assigned(FFormulasCache) and (FFormulasCache.Lock <> not AValue) then
  begin
...
var
  ...
  procedure SetCellData(ACol, ARow: Integer; const AData: string);
  var
    ADataSize: Integer;
    ACell: TcxSSCellRec;
    AFont: TcxSSFontRec;
...
{          FreeMem(FuncRecPtr^.CalcResult.Tokens);
        ...
        if DataType = dtDateTime then
          ...
          begin
            ...
            FFormulasCache.UpdateExternalLinks(FuncRecPtr, DC, DR);
...
begin
  ...
  try
    ...
    if Result then
    begin
      ShowMessage(scxChangePartOfMergeCells);
      Exit;
    end
    else
      Result := True;
    ...
    begin
      ...
      begin
        ...
        begin
          SetCellData(I, J, Copy(S, APos, ALen));
          Inc(APos, ALen);
        end
        else
          Break;
...
function TcxCustomSpreadSheetBook.SpreadSheetDataToClipboardData(
  ASheet: TcxSSBookSheet; const ACells: TRect): string;

  function GetCellData(ACol, ARow: Integer): string;
  var
    ADataSize: Integer;
    ACapacity: Integer;
    APtr: PByteArray;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          with FontPtr^ do
          begin
            WriteBuf(Name[1], Length(Name), True);
        ...
        if DataType = dtDateTime then
          ...
          begin
           ...
           begin
             APtr^[ADataSize - SizeOf(DataType)] := Byte(dtText);
             if Length(Text) > 0 then
               WriteBuf(string(Text)[1], Length(Text), True)
             else
...
{$IFNDEF VCL}
function TcxCustomSpreadSheetBook.WidgetFlags: Integer;
begin
...
begin
  if (ASheet2 > PageCount) or (ASheet2 > PageCount) then Exit;
...
begin
  ...
  if ASheet = FActivePage then
  begin
    if FActivePage = (PageCount - 1) then
      SetActivePage(FActivePage - 1)
    else
    begin
      SetActivePage(FActivePage + 1);
      Dec(FActivePage);
...
{ TcxSSClipboard }
function TcxSSClipboard.GetESSData: string;
{$IFNDEF WINDOWS}
var
...
begin
  ...
  try
    if Clipboard.GetFormat(SCF_ESSCLIPBOARDFORMAT, ClipboardStream) then
    begin
      ClipboardStream.Position := 0;
      SetLength(Result, ClipboardStream.Size);
      if Length(Result) > 0 then
        ClipboardStream.ReadBuffer(Result[1], Length(Result));
    end
    else
      Result := '';
  finally
...
begin
  {$IFDEF WINDOWS}
  SetLength(Result, BufferSize(CF_UNICODETEXT) shr 1);
  GetBuffer(CF_UNICODETEXT, Result[1]);
  {$ENDIF}
end;

procedure TcxSSClipboard.SetESSData(const Value: string);
{$IFNDEF WINDOWS}
var
  ClipboardStream: TMemoryStream;
{$ENDIF}
begin
  ...
  begin
    ...
    ClipboardStream := TMemoryStream.Create;
    try
      ClipboardStream.WriteBuffer(Value[1], Length(Value));
      ClipboardStream.Position := 0;
      ClipBoard.AddFormat(SCF_ESSCLIPBOARDFORMAT, ClipboardStream)
    finally
      ClipboardStream.Free;
    end;
    {$ENDIF}
