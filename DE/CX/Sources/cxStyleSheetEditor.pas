...
uses
  ...
  cxClasses, cxGraphics, cxStyles, ExtDlgs;
...
type
  ...
  TfrmcxStyleSheetEditor = class(TForm)
    ...
    cbFont: TCheckBox;
    cbBitmap: TCheckBox;
    OpenPictureDialog1: {$IFDEF VCL}TOpenPictureDialog{$ELSE}TOpenDialog{$ENDIF};
...
{$ENDIF}
{$IFDEF VCL}
    procedure lbStylesDrawItem(Sender: TObject; Index: Integer; Rect: TRect;
...
begin
  ...
  if OpenPictureDialog1.Execute then
    try
      for I := 0 to lbStyles.Items.Count - 1 do
        if lbStyles.Selected[I] then
          TcxStyle(FStyleList[I]).Bitmap.LoadFromFile(OpenPictureDialog1.FileName);
    finally
      lbStyles.Invalidate;
      UpdateState;
    end;
