...
namespace MySQLDriverCS
{
	...
	public class MySQLSelectCommand
	{
			...
			)
		{
			...
			query += WhereQueryPart(WhereParamFields,WhereNoparamFields,"WHERE");
...
//					for(n=0;n<Fields.Length;n++)
		...
		internal static string WhereQueryPart(object[,] WhereParamFields, string[,] WhereNoparamFields, string prefix)
		{
			string query=""; int n;
			bool Anything=false;
			if((WhereParamFields!=null)&&(WhereParamFields.Length!=0))
			{
				...
				for(int m=0; m<WhereParamFields.GetLength(0); m++)
				{
					...
					{
						query += " "+Field+" "+Operand+" "+MySQLUtils.Escape(Value.ToString())+" ";
					}
					if(n!=0) query+=" AND ";
				}
			}
		...
		)
		{
			...
			// Where
			query += WhereQueryPart(WhereValues,WhereExpressions,"WHERE");
			// Group by
			if(true)
			{
				if(GroupBy!=null)
				{
					...
					// Having
					query += WhereQueryPart(GroupByHavingValues,GroupByHavingExpressions,"HAVING");
				}
			}
			// Order by
			if(OrderBy!=null)
