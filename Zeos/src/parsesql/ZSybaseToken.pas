...
type
  ...
  TZSybaseNumberState = class (TZAbstractObject, IZNumberState)
  private
  ...
  TZSybaseWordState = class (TZWordState)
  public
    constructor Create;
  end;
...
procedure TZSybaseNumberState.CheckExceptionState;
begin
...
procedure TZSybaseNumberState.SetExceptionState(Value: IZTokenizerState);
begin
...
procedure TZSybaseNumberState.SetSeparation(Value: Boolean);
begin
  FSeparation := Value;
end;
...
function TZSybaseNumberState.ReadHexDigits(Stream: TStream): string;
begin
  Result := '';
  FLastChar := #0;
...
function TZSybaseNumberState.ReadDecDigits(Stream: TStream): string;
begin
  Result := '';
  FLastChar := #0;
  while Stream.Read(FLastChar, 1) > 0 do
...
function TZSybaseNumberState.GetNextToken(Stream: TStream; FirstChar: Char;
...
function TZSybaseQuoteState.GetNextToken(Stream: TStream; FirstChar: Char;
  Tokenizer: IZTokenizer): IZToken;
var
...
begin
  ...
  begin
    if ((LastChar = FirstChar) and (ReadChar <> FirstChar) and (FirstChar <> '[')) or ((FirstChar = '[') and (LastChar = ']')) then
    begin
      Stream.Seek(-1, soFromCurrent);
      Break;
    end;
  ...
  if FirstChar in ['"', '['] then
    Result := TZToken.CreateWithType(TT_WORD, ReadStr)
  else Result := TZToken.CreateWithType(TT_QUOTED, ReadStr);
end;
...
function TZSybaseCommentState.GetNextToken(Stream: TStream; FirstChar: Char;
  Tokenizer: IZTokenizer): IZToken;
var
  ReadChar: Char;
  ReadNum: Integer;
...
begin
  ...
  CommentState.SetExceptionState(NumberState);
...
constructor TZSybaseTokenizer.CreateWithBuffer(Buffer: string);
begin
  Create;
  SetBuffer(Buffer);
end;
