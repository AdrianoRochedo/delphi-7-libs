...
uses
  Classes, SysUtils, ZSysUtils, ZClasses, ZDbcIntfs, ZParseIntfs,
  ZDbcResultSetMetadata, ZDbcCachedResultSet, ZDbcCache, ZCompatibility,
  ZSelectSchema, ZGenericSqlToken, ZGenericSelectParser, ZGenericSqlAnalyser;
...
type
  ...
  TZAbstractDatabaseMetadata = class(TInterfacedObject, IZDatabaseMetadata,
    IZExtraDatabaseMetadata)
  private
    FConnection: IZConnection;
    FUrl: string;
    FInfo: TStrings;
  protected
    constructor Create(Connection: IZConnection; Url: string;
      Info: TStrings);

    function CreateColumnInfo(Name: string; SQLType: TZSQLType;
      Length: Integer): IZColumnInfo;
    function CreateVirtualResultSet(ColumnsInfo: IZCollection):
      IZVirtualResultSet;
  ...
  public
    ...
    function GetSelectParser: IZSelectParser; virtual;
...
constructor TZAbstractDatabaseMetadata.Create(Connection: IZConnection;
...
function TZAbstractDatabaseMetadata.CreateVirtualResultSet(
  ColumnsInfo: IZCollection): IZVirtualResultSet;
begin
...
begin
  ColumnsInfo := TZCollection.Create;
...
{**
  Creates a generic select parser object.
  @returns a creates generic select parser object.                                                         
}
function TZAbstractDatabaseMetadata.GetSelectParser: IZSelectParser;
begin
  Result := TZGenericSelectParser.Create;
end;
...
constructor TZVirtualResultSet.CreateWithColumns(ColumnsInfo: IZCollection;
  SQL: string);
begin
