...
uses
  Classes, SysUtils, ZClasses, ZCollections, ZParseIntfs, ZSysUtils,
  ZCompatibility, ZGenericSelectParser, ZSelectSchema, ZGenericSqlAnalyser;
...
type
  ...
  {** Defines a SQL types compatible with JDBC. }
  {
  TZSQLType2 = (st2Bit = -7, st2TinyInt = -6, st2SmallInt = 5, st2Integer = 4,
    st2BigInt = -5, st2Float = 6, st2Real = 7, st2Double = 8, st2Numeric = 2,
    st2Decimal = 3, st2Char = 1, st2VarChar = 12, st2LongVarChar = -1,
    st2Date = 91, st2Time = 92, st2Timestamp = 93, st2Binary = -2,
    st2VarBinary = -3, st2LongVarBinary = -4, st2Null = 0, st2Other = 1111,
    st2Distinct = 2001, st2Struct = 2002, st2Array = 2003, st2Blob = 2004,
    st2Clob = 2005, stRef = 2006);
  }
...
type
  ...
  IZDriverManager = interface(IZInterface)
    ...
    procedure SetLogWriter(_Out: TWriter);
    function GetLogWriter: TWriter;
    procedure PrintLn(_Message: string);
  end;
  ...
  IZConnection = interface(IZInterface)
    ...
    procedure SetAutoCommit(AutoCommit: Boolean);
    ...
    procedure SetCatalog(Catalog: string);
    function GetCatalog: string;

    procedure SetTransactionIsolation(Level: TZTransactIsolationLevel);
  ...
  {** Represents interface to additional database metadata }
  IZExtraDatabaseMetadata = interface(IZInterface)
    ['{E4535765-C66F-4E6C-826A-4BA1D61FCC20}']
    function GetTokenizer: IZTokenizer;
    function GetSelectParser: IZSelectParser;
  ...
  IZStatement = interface(IZInterface)
    ...
    procedure SetMaxFieldSize(Max: Integer);
    function GetMaxRows: Integer;
    procedure SetMaxRows(Max: Integer);
    procedure SetEscapeProcessing(Enable: Boolean);
    function GetQueryTimeout: Integer;
    procedure SetQueryTimeout(Seconds: Integer);
    procedure Cancel;
    ...
    procedure SetFetchSize(Rows: Integer);
    ...
    procedure SetResultSetType(ResultSetType: TZResultSetType);
  ...
  IZPreparedStatement = interface(IZStatement)
    ...
    procedure SetLong(ParameterIndex: Integer; Value: LongInt);
    procedure SetFloat(ParameterIndex: Integer; Value: Single);
    procedure SetDouble(ParameterIndex: Integer; Value: Double);
    procedure SetBigDecimal(ParameterIndex: Integer; Value: Int64);
  ...
  IZCallableStatement = interface(IZPreparedStatement)
    ...
    function GetLong(ParameterIndex: Integer): LongInt;
    function GetFloat(ParameterIndex: Integer): Single;
    function GetDouble(ParameterIndex: Integer): Double;
    function GetBigDecimal(ParameterIndex: Integer): Int64;
  ...
  IZResultSet = interface(IZInterface)
    ...
    function GetLong(ColumnIndex: Integer): LongInt;
    function GetFloat(ColumnIndex: Integer): Single;
    function GetDouble(ColumnIndex: Integer): Double;
    function GetBigDecimal(ColumnIndex: Integer): Int64;
    ...
    function GetLongByName(ColumnName: string): LongInt;
    function GetFloatByName(ColumnName: string): Single;
    function GetDoubleByName(ColumnName: string): Double;
    function GetBigDecimalByName(ColumnName: string): Int64;
    ...
    procedure UpdateLongByName(ColumnName: string; Value: LongInt);
    procedure UpdateFloatByName(ColumnName: string; Value: Single);
    procedure UpdateDoubleByName(ColumnName: string; Value: Double);
    procedure UpdateBigDecimalByName(ColumnName: string; Value: Int64);
  ...
  IZNotification = interface(IZInterface)
    ...
    procedure Listen(Event: string);
    procedure Unlisten(Event: string);
...
procedure TZDriverManager.SetLoginTimeout(Seconds: Integer);
begin
  FLoginTimeout := Seconds;
end;

{**
  Gets a log writer object.
  @return a log writer object.
}
function TZDriverManager.GetLogWriter: TWriter;
begin
  Result := FLogWriter;
end;

{**
  Sets a new log writer object.
  @param _Out a new log writer object.
}
procedure TZDriverManager.SetLogWriter(_Out: TWriter);
begin
  FLogWriter := _Out;
end;

{**
  Prints a message to the driver manager's log.
  @param _Message a message to be printed.
}
procedure TZDriverManager.PrintLn(_Message: string);
begin
  if FLogWriter <> nil then
    FLogWriter.WriteString(_Message);
end;
...
begin
  ...
  if Driver = nil then
    raise EZSQLException.Create('Requested database driver was not found.');
...
constructor EZSQLException.Create(const Msg: string);
begin
...
constructor EZSQLException.CreateWithCode(const ErrorCode: Integer;
   const Msg: string);
begin
  inherited Create(Msg);
...
constructor EZSQLWarning.Create(const Msg: string);
begin
  inherited Create(Msg);
  FErrorCode := -1;
end;
...
constructor EZSQLWarning.CreateWithCode(const ErrorCode: Integer;
   const Msg: string);
begin
  inherited Create(Msg);
  FErrorCode := ErrorCode;
