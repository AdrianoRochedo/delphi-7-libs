...
uses
  Classes, SysUtils, ZSysUtils, ZDbcIntfs, ZDbcMetadata, ZParseIntfs,
...
uses ZInterbaseToken, ZDbcInterbase6Utils, ZInterbaseAnalyser;
...
constructor TZInterbase6DatabaseMetadata.Create(Connection: IZConnection;
...
begin
  Result := '';
  //raise EZSQLException.Create('Driver not capable schemas');
end;
...
begin
  Result := '';
  //raise EZSQLException.Create('Driver not capable catalogs');
end;
...
begin
  ...
  begin
    ...
    if TempResultSet.GetIntByName('RDB$PARAMETER_TYPE') = 0 then
...
var
  TargetResultSet: IZVirtualResultSet;
begin
  TargetResultSet := inherited GetSchemas as IZVirtualResultSet;
  TargetResultSet.SetConcurrency(rcReadOnly);
  TargetResultSet.BeforeFirst;
  Result := TargetResultSet;
end;
...
begin
  ...
  begin
    ...
    TargetResultSet.UpdateString(4, TempResultSet.GetStringByName('RDB$FIELD_NAME'));    //COLUMN_NAME
    ...
    TargetResultSet.UpdateString(13, TempResultSet.GetStringByName('RDB$DEFAULT_VALUE'));   //COLUMN_DEF
...
begin
  ...
  begin
    ...
    begin
      ...
      TempResultSet1 := Connection.CreateStatement.ExecuteQuery(Sql);
...
begin
  ...
  TempResultSet := Connection.CreateStatement.ExecuteQuery(
...
begin
  Result := (_Type = rtScrollInsensitive) and
            (Concurrency = rcReadOnly);
end;
...
begin
  if (Length(Pattern) > 2 * 31) then
    raise EZSQLException.Create('Pattern too long');
  ...
  begin
    ...
    Result := Format('%s = ''%s'' and ', [Column, StrippedPattern]);
  end
  else
  begin
    StrippedPattern := StripEscape(Pattern);
    Result := Format('%s || ''%s'' like ''%s%s%%'' and ',
