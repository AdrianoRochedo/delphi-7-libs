...
uses
  ...
  ZPlainPostgreSqlDriver, ZDbcResultSetMetadata, ZCompatibility,
  ZGenericSelectParser;

type

  {** Implements PostgreSQL ResultSet Metadata. }
  TZPostgreSQLResultSetMetadata = class(TZAbstractResultSetMetadata)
  end;
...
uses Math, ZMatchPattern, ZDbcUtils, ZDbcPostgreSql, ZDbcPostgreSqlUtils,
  ZDbcMySqlUtils, ZPostgreSqlSelectParser;
...
begin
  ...
  ColumnInfo.SetColumnType(SQLType);

  if TypeName = 'money' then
    ColumnInfo.SetCurrency(True)
  else if TypeName = 'name' then
    ColumnInfo.SetPrecision(32)
  else if TypeName = 'interval' then
    ColumnInfo.SetPrecision(32)
  else if TypeName = 'regproc' then
    ColumnInfo.SetPrecision(10)
  else if (TypeName = 'bytea') and Connection.IsOidAsBlob then
    ColumnInfo.SetPrecision(256);

  if SQLType = stUnknown then
  begin
    ColumnInfo.SetColumnType(stString);
    ColumnInfo.SetPrecision(255);
...
function TZPostgreSQLResultSet.CreateResultSetMetadata(SQL: string;
...
begin
  ...
  Result := TZPostgreSQLResultSetMetadata.Create(Metadata, SQL, ColumnsInfo);
end;
...
begin
  ...
  begin
    raise EZSQLException.Create(
      'Live result sets are not supported by this class.');
  end;
  ...
  begin
    ...
    begin
      SetColumnName('');
      SetTableName('');
      SetColumnLabel(StrPas(FPlainDriver.GetFieldName(FQueryHandle, I)));
      SetColumnDisplaySize(0);
      SetScale(0);
      ...
      SetPrecision(Max(Max(FPlainDriver.GetFieldMode(FQueryHandle, I) - 4,
        FPlainDriver.GetFieldSize(FQueryHandle, I)), 0));

      if (GetColumnType = stString) or (GetColumnType = stUnicodeString) then
      begin
        if (GetPrecision = 0) or (GetColumnLabel = 'expr') then
          SetPrecision(255);
      end;
    end;
...
{$IFNDEF DISABLE_CHECKING}
  ...
  if (RowNo < 1) or (RowNo > LastRowNo) then
    raise EZSQLException.Create('Row data is not accessable.');
{$ENDIF}
...
function TZPostgreSQLResultSet.GetLong(ColumnIndex: Integer): LongInt;
begin
{$IFNDEF DISABLE_CHECKING}
  CheckColumnConvertion(ColumnIndex, stLong);
{$ENDIF}
  Result := LongInt(StrToInt64Def(GetString(ColumnIndex), 0));
end;
...
function TZPostgreSQLResultSet.GetBigDecimal(ColumnIndex: Integer): Int64;
begin
{$IFNDEF DISABLE_CHECKING}
...
{$ENDIF}
  Result := Str2Bytes(GetString(ColumnIndex));
end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      BlobOid := StrToIntDef(StrPas(FPlainDriver.GetValue(FQueryHandle,
        RowNo - 1, ColumnIndex - 1)), 0);
    end else
      BlobOid := 0;
...
begin
  ...
  begin
    if (BlobSize - Position) < 4096 then
