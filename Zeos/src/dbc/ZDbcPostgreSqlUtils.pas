...
uses Classes, SysUtils, ZDbcIntfs, ZPlainPostgreSqlDriver,
  ZDbcResultSetMetadata, ZDbcPostgreSql;
...
{**
  ...
  @paramm Handle a PostgreSQL connection reference.
}
procedure CheckPostgreSQLError(PlainDriver: IZPostgreSQLPlainDriver;
...
function IsNumber(Value: TZSQLType): boolean;
begin
  Result := (Value = stByte) or (Value = stShort) or (Value = stInteger)
    or (Value = stLong) or (Value = stFloat) or (Value = stDouble)
    or (Value = stBigDecimal);
end;
...
begin
  ...
  if ErrorMessage <> '' then
    raise EZSQLException.Create(Format('SQL Error: %s', [ErrorMessage]));
end;
