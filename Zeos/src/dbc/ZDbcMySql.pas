...
type
  ...
  public
    constructor Create(PlainDriver: IZMySQLPlainDriver; HostName: string;
...
uses ZSysUtils, ZDbcUtils, ZDbcMySqlStatement, ZMySqlToken, ZDbcMySqlUtils,
 ZDbcMySqlMetadata;
...
begin
  ...
  try
    ...
    Result := TZMySQLConnection.Create(PlainDriver, HostName, Port, Database,
      UserName, Password, TempInfo);
  finally
    TempInfo.Free;
  end;
...
constructor TZMySQLConnection.Create(PlainDriver: IZMySQLPlainDriver;
...
procedure TZMySQLConnection.StartTransactionSupport;
begin
  if (TransactIsolationLevel = tiNone) or (AutoCommit) then
  begin
    FPlainDriver.ExecQuery(FHandle, 'SET AUTOCOMMIT=1');
    CheckMySQLError(FPlainDriver, FHandle);
  end
  else
  begin
    FPlainDriver.ExecQuery(FHandle, 'SET AUTOCOMMIT=0');
    CheckMySQLError(FPlainDriver, FHandle);
    case TransactIsolationLevel of
      tiReadUncommitted:
        FPlainDriver.ExecQuery(FHandle,
...
begin
  ...
  { Sets use_result option }
  FUseResult := StrToBoolEx(Info.Values['useresult']);
  ...
  begin
    CheckMySQLError(FPlainDriver, FHandle);
    raise EZSQLException.Create('Cannot connect to SQL server.');
  end;
...
function TZMySQLConnection.CreateStatement: IZStatement;
begin
  if IsClosed then Open;
  Result := TZMySQLStatement.Create(FPlainDriver, Self, FHandle);
end;
...
function TZMySQLConnection.PrepareStatement(SQL: string): IZPreparedStatement;
begin
  if IsClosed then Open;
  Result := TZMySQLPreparedStatement.Create(FPlainDriver, Self, SQL, FHandle);
end;
...
begin
  ...
  begin
    FPlainDriver.ExecQuery(FHandle, 'COMMIT');
    CheckMySQLError(FPlainDriver, FHandle);
  end;
end;
...
begin
  if (TransactIsolationLevel <> tiNone) and not Closed then
  begin
    FPlainDriver.ExecQuery(FHandle, 'ROLLBACK');
    CheckMySQLError(FPlainDriver, FHandle);
  end;
end;
...
function TZMySQLConnection.GetMetadata: IZDatabaseMetadata;
var
  Url: string;
begin
  Url := Format('zdbc:%s://%s:%d/%s?UID=%s;PWD=%s;%s',
    [FPlainDriver.GetProtocol, Hostname, Port, Database, User, Password,
    ComposeString(Info, ';')]);
  Result := TZMySQLDatabaseMetadata.Create(Self, Url, Info);
end;
...
function TZMySQLConnection.IsUseResult: Boolean;
begin
  Result := FUseResult;
end;
