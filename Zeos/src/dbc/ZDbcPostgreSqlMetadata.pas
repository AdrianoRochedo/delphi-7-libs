...
uses
  ...
  ZParseIntfs, ZDbcResultSetMetadata, ZCompatibility, ZDbcPostgreSqlUtils,
  ZGenericSqlAnalyser;
...
type
  ...
  protected
    function GetColumnInfo(Table, Name: string; SQLType: TZSQLType;
      Length: Integer): IZColumnInfo;
    function GetVirtualResultSet(ColumnsInfo: IZCollection): TZVirtualResultSet;
...
uses Math, ZDbcUtils, ZPostgreSqlToken, ZDbcPostgreSql, ZPostgreSqlAnalyser;
...
constructor TZPostgreSQLDatabaseMetadata.Create(Connection: IZConnection;
...
begin
  ...
  begin
    ...
    if ReturnTypeType = 'c' then
    begin
      ColumnsRS := Connection.CreateStatement.ExecuteQuery(
...
var
  ...
  Permissions, PermissionsExp: TZStringList;
begin
  if Table = '' then
    Table := '%';
  ...
  Permissions := TZStringList.Create;
  PermissionsExp := TZStringList.Create;
...
function TZPostgreSQLDatabaseMetadata.GetColumnInfo(Table, Name: string;
  SQLType: TZSQLType; Length: Integer): IZColumnInfo;
begin
  ...
  begin
    SetTableName(Table);
    SetColumnName(Name);
    SetColumnType(SQLType);
    SetColumnDisplaySize(Length);
  end;
end;

function TZPostgreSQLDatabaseMetadata.GetVirtualResultSet(
  ColumnsInfo: IZCollection): TZVirtualResultSet;
begin
...
begin
  ...
  if not ResultSet.Next then
    raise Exception.Create('postgresql.unexpected'); //CHANGE IT!
...
begin
  Result := (Connection as IZPostgreSQLConnection).GetTypeNameByOid(Oid);
end;
...
begin
  Result := PostgreSQLToSQLType(Connection as IZPostgreSQLConnection,
    TypeName);
end;
...
begin
  ...
  begin
    ...
    From := ' FROM pg_catalog.pg_namespace n, pg_catalog.pg_namespace n2, '+
      'pg_catalog.pg_trigger t, pg_catalog.pg_trigger t1, pg_catalog.pg_class c, '+
      'pg_catalog.pg_class c2, pg_catalog.pg_class ic, pg_catalog.pg_proc p1, '+
      'pg_catalog.pg_proc p2, pg_catalog.pg_index i, pg_catalog.pg_attribute a ';
    Where := ' AND c.relnamespace = n.oid AND c2.relnamespace=n2.oid ';
    if PrimarySchema <> ''then
      Where := Where + ' AND n.nspname = ''' + EscapeQuotes(PrimarySchema) + ''' ';
  ...
  begin
    ...
    From := ' FROM pg_trigger t, pg_trigger t1, pg_class c, pg_class c2, pg_class ic, '+
            'pg_proc p1, pg_proc p2, pg_index i, pg_attribute a ';
  end;

  SQL := Select	+ 'c.relname as prelname, '+
  ...
  if PrimaryTable <> '' then
    SQL := SQL + 'AND c.relname=''' + EscapeQuotes(PrimaryTable) + ''' ';
  ...
  begin
    ...
    TargetResultSet.UpdateString(13, ResultSet.GetString(6)); //PK_NAME
