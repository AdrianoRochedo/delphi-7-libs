...
type
  ...
  procedure isc_encode_sql_date_stub(tm_date: PCTimeStructure; ib_date: PISC_DATE);
   {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_encode_sql_date');
  end;

  procedure isc_encode_sql_time_stub(tm_date: PCTimeStructure; ib_time: PISC_TIME);
    {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_encode_sql_time');
  end;

  procedure isc_encode_timestamp_stub(tm_date: PCTimeStructure; ib_timestamp: PISC_TIMESTAMP);
    {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_encode_sql_timestamp');
  end;

  procedure isc_decode_sql_date_stub(ib_date: PISC_DATE; tm_date: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_decode_sql_date');
  end;

  procedure isc_decode_sql_time_stub(ib_time: PISC_TIME; tm_date: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_decode_sql_time');
  end;

  procedure isc_decode_timestamp_stub(ib_timestamp: PISC_TIMESTAMP; tm_date: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  begin
    raise Exception.Create('This version is not is no supported isc_decode_timestamp');
  end;
...
initialization
  LibraryLoader := TZInterbaseNativeLibraryLoader.Create(
    [WINDOWS_DLL_LOCATION, LINUX_DLL_LOCATION]);
finalization
