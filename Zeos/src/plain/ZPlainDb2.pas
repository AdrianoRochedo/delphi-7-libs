...
function Db2LoadLibrary: Boolean;

var
  DllLocation: string = DEFAULT_DLL_LOCATION;
  DllHandle: THandle = 0;
...
begin
  ...
  if DllHandle <> 0 then
  begin
    SQLAllocConnect := GetProcAddress(DllHandle, 'SQLAllocConnect');
    SQLAllocEnv := GetProcAddress(DllHandle, 'SQLAllocEnv');
    SQLAllocStmt := GetProcAddress(DllHandle, 'SQLAllocStmt');
    SQLAllocHandle := GetProcAddress(DllHandle, 'SQLAllocHandle');
    SQLBindCol := GetProcAddress(DllHandle, 'SQLBindCol');
    SQLCancel := GetProcAddress(DllHandle, 'SQLCancel');
    SQLColAttribute := GetProcAddress(DllHandle, 'SQLColAttribute');
    SQLConnect := GetProcAddress(DllHandle, 'SQLConnect');
    SQLDescribeCol := GetProcAddress(DllHandle, 'SQLDescribeCol');
    SQLDisconnect := GetProcAddress(DllHandle, 'SQLDisconnect');
    SQLError := GetProcAddress(DllHandle, 'SQLError');
    SQLExecDirect := GetProcAddress(DllHandle, 'SQLExecDirect');
    SQLExecute := GetProcAddress(DllHandle, 'SQLExecute');
    SQLFetch := GetProcAddress(DllHandle, 'SQLFetch');
    SQLFreeConnect := GetProcAddress(DllHandle, 'SQLFreeConnect');
    SQLFreeEnv := GetProcAddress(DllHandle, 'SQLFreeEnv');
    SQLFreeStmt := GetProcAddress(DllHandle, 'SQLFreeStmt');
    SQLCloseCursor := GetProcAddress(DllHandle, 'SQLCloseCursor');
    SQLGetCursorName := GetProcAddress(DllHandle, 'SQLGetCursorName');
    SQLGetData := GetProcAddress(DllHandle, 'SQLGetData');
    SQLNumResultCols := GetProcAddress(DllHandle, 'SQLNumResultCols');
    SQLPrepare := GetProcAddress(DllHandle, 'SQLPrepare');
    SQLRowCount := GetProcAddress(DllHandle, 'SQLRowCount');
    SQLSetCursorName := GetProcAddress(DllHandle, 'SQLSetCursorName');
    SQLSetParam := GetProcAddress(DllHandle, 'SQLSetParam');
    SQLTransact := GetProcAddress(DllHandle, 'SQLTransact');
    SQLEndTran := GetProcAddress(DllHandle, 'SQLEndTran');
    SQLFreeHandle := GetProcAddress(DllHandle, 'SQLFreeHandle');
    SQLGetDiagRec := GetProcAddress(DllHandle, 'SQLGetDiagRec');
    SQLGetDiagField := GetProcAddress(DllHandle, 'SQLGetDiagField');
    SQLCopyDesc := GetProcAddress(DllHandle, 'SQLCopyDesc');
    SQLGetDescField := GetProcAddress(DllHandle, 'SQLGetDescField');
    SQLGetDescRec := GetProcAddress(DllHandle, 'SQLGetDescRec');
    SQLSetDescField := GetProcAddress(DllHandle, 'SQLSetDescField');
    SQLSetDescRec := GetProcAddress(DllHandle, 'SQLSetDescRec');
    { New added functions }
    SQLSetConnectAttr := GetProcAddress(DllHandle, 'SQLSetConnectAttr');
    SQLSetConnectOption := GetProcAddress(DllHandle, 'SQLSetConnectOption');
    SQLSetStmtAttr := GetProcAddress(DllHandle, 'SQLSetStmtAttr');
    SQLSetStmtOption := GetProcAddress(DllHandle, 'SQLSetStmtOption');
    SQLGetSubString := GetProcAddress(DllHandle, 'SQLGetSubString');
    SQLGetLength := GetProcAddress(DllHandle, 'SQLGetLength');
    SQLGetPosition := GetProcAddress(DllHandle, 'SQLGetPosition');
    SQLBindParameter := GetProcAddress(DllHandle, 'SQLBindParameter');
    SQLParamData := GetProcAddress(DllHandle, 'SQLParamData');
    SQLPutData := GetProcAddress(DllHandle, 'SQLPutData');
