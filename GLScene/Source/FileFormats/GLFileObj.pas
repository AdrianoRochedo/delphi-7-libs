...
uses {$IFDEF STATS}Dialogs, {$ENDIF} Classes, SysUtils, GLScene,
         ...
         for i:=0 to FPolygonVertices.Count-1 do begin
            ...
            for j:=0 to FPolygonVertices[i]-1 do begin
               if j>1 then begin
                  ...
                  aList.Add(vertexList[VertexIndices[n-1]]);
                  aList.Add(vertexList[VertexIndices[n]]);
               end;
               Inc(n);
            end;
         end;
...
begin
  ...
  while not FEof do
    begin
      ...
      if FLine[1]='$' then continue; { Skip alternate comment }
