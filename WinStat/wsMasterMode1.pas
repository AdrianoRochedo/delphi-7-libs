...
Type
  ...
  Protected
    ...
    CoefList    : TList;          // Lista com as saídas dos coeficientes
...
{<<<<<
  gOutPut.WriteTitle('========== Análise'+Manager.MFrame[Manager.DataIndex].MLab+' ==========',clBlack);
  ...
  if Manager.FWeightVar <> '' then
    ...
    WVar:=True;
{<<<<<
    gOutPut.WriteTitle('Ponderação pela variável: '+Manager.FWeightVar,clBlack);
}
    end
  else
    WVar:=False;
{<<<<<
  if FManager.FCondition <> '' then
    gOutPut.WriteTitle('Observações incluídas pela condição: '+Manager.FCondition,clBlack);
  gOutPut.WriteLine;
}
...
{<<<<<
  ...
  // Imprime contrastes ou matrizes de codificação
  if Manager.Options[ord(cmVar_Contrastes)] then
    begin
{<<<<<
    gOutPut.WriteCenter('Matrizes Para Codificação da Matriz X');
}
    PrintCodeMat;
    end;
    ...
    if T.FTest then     // se foi realizado o teste F
      ...
      for nk:=1 to NResp do
        ...
        for i:=1 to LV.NCols do
          T.LSMeans.Struct.AddColEx(TwsNumeric.Create('Cov_'+IntToStr(i),'Covariâncias',12,7));
        for i:=1 to mk.NRows do
          begin
...
{<<<<<
  if DS.Struct.Col[IndFator].ColType = dtQuant then
    gOutPut.Warning('Fator '+Fator+' é quantitativo. Regressão Polinomial'+
                    ' pode ser uma análise mais adequada para este fator');
}
...
{<<<<<
    if gLink then
      gOutPut.WriteLink('Veja mais detalhes sobre Comparações Múltiplas',
...
{<<<<<
    if gLink then
      gOutPut.WriteLink('Veja mais detalhes sobre Regressão Polinomial',
...
begin
  ...
  for k := 1 to NResp do
    ...
    CoefTable.Struct.AddNumeric('F', 'Valor da estatística F',12,7);              {4}
    CoefTable.Struct.AddNumeric('Valor_p', 'Nível mínimo de significância',11,4); {5}
    CoefTable.Struct.AddQualitative('Sig','Significância do teste F',5);          {6}
...
{<<<<<
      if Old=ColOp[k] then
         gOutPut.Warning('Coluna '+IntToStr(k)+' não pode ser operada')
}
      end;
...
begin
  ...
  for k := 1 to ny do
    ...
    CoefTable.Struct.AddNumeric('t', 'Valor da estatística t',12,7);              {3}
    CoefTable.Struct.AddNumeric('Valor_p', 'Nível mínimo de significância',11,4); {4}
    CoefTable.Struct.AddQualitative('Sig','Significância do teste F',5);          {5}
...
begin
  for k:=1 to NResp do
    ...
    with CoefTable do
      ...
      Struct.AddColEx(TwsNumeric.Create('E_Padrao','Erro padrão da estimativa',12,7)); //2

      Struct.AddColEx(TwsNumeric.Create('T','Estatística T para hipótese',10,5));      //3

      Struct.AddColEx(TwsNumeric.Create('Valor_p','Valor p para hipótese Ho',10,5));   //4

      Col:=TwsQualitative.Create('Sig','Significância do teste ao nível 5% ou 1%',6);  //5
      With TwsQualitative(Col) Do
        Begin
        AddLevel('NS');
        AddLevel('5%');
...
{ Objetivo
    ...
    se n é o número de níveis em questão, retorna uma matriz de dimensão n x (n-1) onde na
    coluna i (i=1, ..., n-1), a posição i vale 1 e nas demias vale 0.
  Campos modificados
    Nenhum
  Métodos chamados
    Nenhum
  ...
  Try
    ...
    if (FFacIdx<>nil) then
      ...
      for i := 0 to FListFrame.Count-1 do
        ...
        DSMean.Struct.AddNumeric('_n', 'Número de observações para média',7,0);
...
{ Objetivo
    ...
    Os modelos disponíveis são
      * Regressão univariada modelo completo
      * Regressão univariada passo a passo, método stepwise
      * Regressão univariada passo a passo, backward elimination
      * Regressão univariada todas as regressões possíveis
      * Regressão univariada passo a passo, método forward selection
      * Regressao univariada ponderada
  Parâmetros
    MSelect: Métodos de seleção de variáveis
    ialpha,
    ...
    Case FClassType of
      mtUnivRegComp:     { Regressão univariada modelo completo }
        ...
        with TwsUReg(FModel) do
          begin
          //gOutPut.WriteTitle('Regressão Univariada: Modelo Completo',clBlack); <<<<<
          For k := 1 to NResp do
            Begin
            RespVar := k;
{<<<<<
            gOutPut.WriteTitle('Variável Resposta: '+
      ...
      mtStepwiseSelect:   { Regressão univariada passo a passo, método stepwise }
        Begin
{<<<<<
        gOutPut.WriteTitle('Seleção de modelos - Stepwise Selection',clBlack);
}
...
{<<<<<
        if gLink then
          gOutPut.WriteLink('Veja mais detalhes sobre o método Stepwise Selection',
      ...
      mtBackwardEliminat: { Regressão univariada passo a passo, backward elimination }
        Begin
{<<<<<
        gOutPut.WriteTitle('Seleção de modelos - Backward Elimination',clBlack);
}
...
{<<<<<
        if gLink then
          gOutPut.WriteLink('Veja mais detalhes sobre o método Backward Elimination',
      ...
      mtAllReg:           { Regressão univariada todas as regressões }
        Begin
{<<<<<
        gOutPut.WriteTitle('Seleção de modelos - Todas as Regressões Possíveis',clBlack);
}
...
{<<<<<
        if gLink then
          gOutPut.WriteLink('Veja mais detalhes sobre o método Todas as Regressões',
      ...
      mtForwardSelect:    { Regressão univariada passo a passo, método forward }
        Begin
{<<<<<
        gOutPut.WriteTitle('Seleção de modelos - Forward Selection',clBlack);
}
...
{<<<<<
        if gLink then
          gOutPut.WriteLink('Veja mais detalhes sobre o método Forward Selection',
...
begin
  if FFacIdx <> nil then
    ...
    if (FRIdx.Len=1) or FUnivariado then  { Analise de regressão univariada }
      case VarSelect of
...
{ Objetivo:
  ...
  Parâmetros:
    ...
    YCol :  Coluna sobre a qual será executado o teste
...
var
  ...
  RCol,nt       : Integer;
  Sig,Equal     : Boolean;
  Groups,                      { Groups armazena os grupos homogênos }
  RValue, Tab   : TwsGeneral;    { RValue armazena os valores críticos }
  Comp          : TwsDataSet;    { Conjunto para armazenar as comparações realizadas }
  Lin           : TwsVec;
  Asc, CS       : TwsLIVec;
...
begin
  ...
  with RValue do
    begin
    Name:='Inf_Teste';
    MLab := 'Comparações múltiplas de médias - Variável: '+ Y.ColName[YCol];
    RowName[1]:='Valor';
...
{<<<<<
  ...
  Comp.MLab := 'Comparações realizadas';
  ...
  Groups.MLab := 'Grupos homogêneos. Médias com o mesmo nível não diferem entre si';
  Groups.Name := 'Sequenc';
  Groups.PrintOptions.ColWidth := 10;

  Tit:= 'Diferenças Mínimas Significativas (DMS) ao nível ' + FloatToStr(100*Level) + '%';
  ...
  case MRTest of
    ...
    mtScheffe:
      begin
      RValue.Name := 'Scheffe';
      RValue.MLab:=Tit + ' - Teste de Schèffé';
...
begin
{<<<<<
  gOutPut.WriteTitle('Análise de Regressão Polinomial - Variável: '+Means.ColName[YCol],clBlack);
  gOutPut.WriteLine;
}
  ...
  { Tabela com a análise da variação }
  ...
  AnovaTable.Struct.AddNumeric('F','Valor da estatística F',12,7);
  AnovaTable.Struct.AddNumeric('Valor_p','Valor p para a hipótese Ho',11,4);
  AnovaTable.Struct.AddQualitative('Sig','Significância do teste ao nível 5% ou 1%',6);
  ...
  if Options[ord(cmVar_GrafRP)] then
    ...
    for i := 2 to Bet.NRows do
      ...
      // Plota v contra w (Funcao) e y contra x (Pontos) no mesmo gráfico.
      ...
      if k = 0 then
         ...
         Graf.Grafico.Title.Text.Add('Equação de Grau '+IntToStr(i-1)+' '+Means.MLab);
...
{<<<<<
         ...
         begin
         {$IFDEF DEBUG}
         ShowMessage('Erro na geração dos gráficos: Código ' + intToStr(k));
         {$ENDIF}
         end;
      End; // for i
    v.free; w.free;
...
begin
  ...
  with mContr do
    ...
    Struct.AddColEx(TwsNumeric.Create('F','Valor da Estatística F',12,7));          //4

    Struct.AddColEx(TwsNumeric.Create('Valor_p','Nível Mínimo de Significância',11,4)); //5

    Col:=TwsQualitative.Create('Sig','Significância do teste ao nível 5% ou 1%',6);  //6
    With TwsQualitative(Col) Do
      Begin
      AddLevel('NS');
      AddLevel('5%');
    ...
    Struct.AddColEx(TwsNumeric.Create('Lim_Inf','Limite Inferior Int. Confiança')); //7
    Struct.AddColEx(TwsNumeric.Create('Lim_Sup','Limite Superior Int. Confiança')); //8
    end;
    ...
    If v[5]>=0.05                            // Significancia da estatistica
      ...
      Else
        If v[5]>=0.01
          ...
          Else
            v[6]:=2;
    v[7] := v[1] - aux*sqrt(CVar*sqc);       // Extremo inferior do intervalo de confiança
    v[8] := v[1] + aux*sqrt(CVar*sqc);       // Extremo superior do intervalo de confiança
    mContr.MAdd(v)
    end;
