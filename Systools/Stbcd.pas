...
{!!.01  Added FloatFormBcd function}
...
var
  E : EStBCDError;
begin
  E := EStBCDError.CreateResTP(Code);                          {!!.04}
  E.ErrorCode := Code;
  raise E;
end;
...
begin
  C := 0;
  for I := 0 to MantissaDigits do begin {!!.03}
    T := UB2[I]+UB1[I]+C;
    if T > 9 then begin
      C := 1;
      dec(T, 10);
...
var
  I : Integer;
begin
  for I := 0 to MantissaDigits do {!!.03}
...
begin
  ...
  for I := MantissaDigits downto 0 do {!!.03}
    if UB[I] <> 0 then begin
      Shift := MantissaDigits-I;
      if Shift >= E then begin                   {!!.03}
        {number disappears}
        E := 0;
        FillChar(UB[0], SigDigits, 0);
      end else if Shift <> 0 then begin
        dec(E, Shift);
        move(UB[0], UB[Shift], SigDigits-Shift); {!!.03}
        FillChar(UB[0], Shift, 0);               {!!.03}
      end;
      Exit;
    end;
...
{$ENDIF}
  C : Byte;
begin
  if Start > MantissaDigits then begin {!!.03}
    Start := SigDigits;                {!!.03}
    C := 0;                            {!!.03}
  end else                             {!!.03}
    C := UB[Start];
...
begin
  if Shift > MantissaDigits then {!!.03}
    {UB disappears when shifted}
    FillChar(UB[0], SigDigits+1, 0) {!!.03}

  else if Shift > 0 then begin {!!.03}
    Move(UB[Shift], UB[0], SigDigits+1-Shift); {!!.03}
    FillChar(UB[SigDigits+1-Shift], Shift, 0); {!!.03}
  end;
end;
...
begin
  if B1[0] = 0 then
    ...
    if S1 <> S2 then begin
      ...
      if UB2[SigDigits] = 0 then {!!.03}
        RoundMantissa(UB2, 0);   {!!.03}
      if UB2[SigDigits] <> 0 then begin
        {temporary overflow}
        RoundMantissa(UB2, 1);
...
begin
  S := StrExpBcd(B, 0);
  if (DecimalSeparator <> '.') then begin                              {!!.03}
    while (pos(DecimalSeparator, S) > 0) do                            {!!.03}
      S[pos(DecimalSeparator, S)] := '.';                              {!!.03}                       {!!.03}
  end;                                                                 {!!.03}
  Val(S, Result, Code);
end;
...
(*                                                                     {!!.01}
        ...
        ',' :                                                          {!!.01}
          ThousandSep := True;                                         {!!.01}
                                                                       {!!.01}
        '.' :                                                          {!!.01}
          if DecimalIndex = -1 then                                    {!!.01}
            DecimalIndex := DigitCount;                                {!!.01}
*)                                                                     {!!.01}
        ...
        if Ch = ThousandSeparator then                                 {!!.01}
          ThousandSep := True;                                         {!!.01}
                                                                       {!!.01}
        if Ch = DecimalSeparator  then                                 {!!.01}
          if DecimalIndex = -1 then                                    {!!.01}
            DecimalIndex := DigitCount;                                {!!.01}
      end;
...
{        ',', '.' :}                                                   {!!.01}
          {these characters are automatically inserted in StoreDigit}  {!!.01}
        ...
        {these characters are automatically inserted in StoreDigit};   {!!.01}
        if not (Ch in [ThousandSeparator, DecimalSeparator]) then      {!!.01}
          StoreChar(Ch);
      end;
...
{$ENDIF}
    ...
    if TB[2*MantissaDigits] <> 0 then begin
      PB := PUnpBcd(@TB[MantissaDigits]);   {!!.03}
      Digits := 0;                          {!!.03}
    end else begin
      PB := PUnpBcd(@TB[MantissaDigits-1]); {!!.03}
      Digits := -1;                         {!!.03}
    end;
    RoundMantissa(PB^, 0);                  {!!.03}
...
begin
  if B[0] = 0 then
    ...
    if Places >= ActPlaces then
      ...
      RoundMantissa(UB, ActPlaces-Places);
      if UB[SigDigits] <> 0 then begin
        ShiftMantissaDown(UB, 1);
        inc(Exponent);
      end else if IsZeroMantissa(UB) then
...
begin
  ...
  if Exponent < ExpBias+1 then begin
    Digits := Places-(ExpBias-Exponent);
    if Digits < 0 then
      Digits := 0;
  end else
    Digits := ActWidth;
...
{$ENDIF}
  ...
  if Width > ActWidth then begin
    ...
    FillChar(Result[1], Width-ActWidth, ' ');
    O := Width-ActWidth+1;
  end else
    O := 1;
  ...
  if Digits <> 0 then begin
    ...
    if UB[I] = 0 then
      dec(I);
    while (Digits > 0) and (O <= Width) do begin
...
function FloatFormBcd(const Mask : string; B : TBCD;                         {!!.01}
                      const LtCurr, RtCurr : string;
                      Sep, DecPt : AnsiChar) : string;
  {-Returns a formatted string with digits from B merged into the Mask}
type
...
initialization
  ZeroBcd := FastVal('0.0');
  MinBcd := ValBcd('-9'+DecimalSeparator+'9E+63');                     {!!.01}
  MaxBcd := ValBcd('9'+DecimalSeparator+'9E+63');                      {!!.01}
