...
type
  ...
  TBitIterateFunc = function (Container : TStBits;
                              N : LongInt;
...
{$ENDIF}
    ...
    if Up then begin
      ...
      N := MaxLong(From, 0);
      F := MinLong(btBlockSize, N shr 3);
      O := ShortInt(N) and 7;
      B := btByte(F)^;

      while (N <= FMax) and (O <= 7) do begin
