...
function InvTan2(X, Y : Double) : Double;
begin
...
begin
  ...
  SP := SunPosPrim(UT);
  SunRA := InvTan2(SP.SunX, SP.SunY) * radcor;
  SunDC := InvSin(SP.SunZ / SP.RV) * radcor;
...
{Earth's orbital eccentricity}
  ...
  Result.Plx := InvSin(6378.14/MoonDst) * radcor;
  Result.Dia := 358473400 / MoonDst * 2 / 3600;                        {!!.04}

  S1 := sin(MoonLong) * cos(OB2000) - tan(MoonLat) * sin(OB2000);
  C1 := cos(MoonLong);
  Result.RA := InvTan2(C1, S1) * radcor;
...
begin
  ...
  SP := SunPosPrim(UT);
  Result.RA := InvTan2(SP.SunX, SP.SunY) * radcor;
  Result.DC := InvSin(SP.SunZ / SP.RV) * radcor;
end;

{-----------------------------------------------------------------------------}

{revised for 1.05}
function RiseSetPrim(LD : TStDate; Longitude, Latitude, H0 : Double;
                     PR : TStPosRecArray) : TStRiseSetRec;
...
{check if object never rises/sets}
  ...
  if (abs(HA) > 1) then begin
    if (Latitude - 90) > PR[2].DC then begin                           {!!.05}
{    if (90 - Latitude) >= PR[2].DC then begin}                        {!!.05}
{object never rises}
      Result.ORise := -2;
      Result.OSet  := -2;
    end else begin
...
{refine rise time by interpolation/iteration}
  ...
  repeat
    ...
    Alt := InvSin(sin(LatR) * sin(DC) + cos(LatR) * cos(DC) * cos(HA));
    DeltaR := ((Alt - H0) * radcor) / (360 * cos(DC) * cos(LatR) * sin(HA));
    TRise := TRise + DeltaR;
    Inc(ICount);                                                       {!!.05}
    if (ICount > 3) and (abs(DeltaR) >= 0.0005) then begin             {!!.05}
      if (ICount = 4) then begin                                       {!!.05}
        TV1 := TRise;                                                  {!!.05}
        A1  := (Alt-H0) * radcor;                                      {!!.05}
      end else if (ICount = 5) then begin                              {!!.05}
        TV2 := TRise;                                                  {!!.05}
        A2  := (Alt-H0) * radcor;                                      {!!.05}
                                                                       {!!.05}
        TRise := TV1 + (A1 / A2) * (TV1 - TV2);                        {!!.05}
        break;                                                         {!!.05}
      end;                                                             {!!.05}
    end;                                                               {!!.05}
  until (abs(DeltaR) < 0.0005); {0.0005d = 0.72 min}                   {!!.05}
...
{refine set time by interpolation/iteration}
  ...
  repeat
    ...
    TSet := TSet + DeltaS;
    Inc(ICount);                                                       {!!.05}
    if (ICount > 3) and (abs(DeltaS) >= 0.0005) then begin             {!!.05}
      if (ICount = 4) then begin                                       {!!.05}
        TV1 := TSet;                                                   {!!.05}
        A1  := (Alt-H0) * radcor;                                      {!!.05}
      end else if (ICount = 5) then begin                              {!!.05}
        ...
        A2  := (Alt-H0) * radcor;
                                                                       {!!.05}
        TSet := TV1 + (A1 / A2) * (TV1 - TV2);                         {!!.05}
        break;                                                         {!!.05}
      end;                                                             {!!.05}
    end;                                                               {!!.05}
  until (abs(DeltaS) < 0.0005); {0.0005d = 0.72 min}                   {!!.05}
...
begin
  ...
  if CheckDate(UT) then begin                                          {!!.05}
    UT.D := UT.D + 2;                                                  {!!.05}
    if (not CheckDate(UT)) then begin                                  {!!.05}
      Result.ORise := -4;                                              {!!.05}
      Result.OSet  := -4;                                              {!!.05}
      Exit;                                                            {!!.05}
    end else                                                           {!!.05}
...
begin
  ...
  for I := 1 to 3 do begin
    UT.D := LD + I-1;
    RP[I] := SunPos(UT);
    if (I > 1) then begin                                              {!!.04}
      if RP[I].RA < RP[I-1].RA then                                    {!!.04}
        RP[I].RA := RP[I].RA + 360.0;                                  {!!.04}
    end;                                                               {!!.04}
  end;
  Result := RiseSetPrim(LD, Longitude, Latitude, H0, RP);
end;
...
(*
  if (LD < MinDate+1) or (LD > MaxDate) then begin                     {!!.05}
    Result.ORise := -4;                                                {!!.05}
    Result.OSet  := -4;                                                {!!.05}
    Exit;                                                              {!!.05}
  end;                                                                 {!!.05}
