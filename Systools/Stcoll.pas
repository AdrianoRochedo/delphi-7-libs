...
type
  ...
  TPointerArray = array[0..(MaxBlockSize div SizeOf(Pointer))-1] of Pointer;
  ...
  protected
    ...
    function conStoresPointers : boolean; override;
  {.Z-}
  public
...
procedure TStCollection.Assign(Source: TPersistent);
  begin
    ...
    {$ENDIF}
      ...
      if not conAssignPointers(Source, AssignData) then
    ...
    if Index >= N.pdStart then
      ...
      repeat
        ...
        if Index < Start then begin
          ...
          colCachePage := N;
          N.pdPage^[Index-Start] := Data;   {!!.B1}
          N.pdCount := Index-Start+1;       {!!.B1}
          Exit;
        end;
        N := TPageDescriptor(N.FNext);
      until not Assigned(N)
...
procedure TStCollection.conForEachPointer(Action : TIteratePointerFunc;
                                          OtherData : pointer);
var
  I : Integer;
  N : TPageDescriptor;
...
function TStCollection.conStoresPointers : boolean;
  begin
    Result := true;
  end;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    if (Count = 0) then begin                                          {!!.03}
      Result := -1;                                                    {!!.03}
      Exit;                                                            {!!.03}
    end;                                                               {!!.03}
    N := colCachePage;
    if Compare(Data, N.pdPage^[0]) >= 0 then begin
      {search up}
      repeat
...
begin
  with N do
    if pdCount = 0 then begin
      Result := SCSPageEmpty;
      PageIndex := 0;
    end else if Compare(AData, pdPage^[0]) < 0 then begin
      Result := SCSLessThanThisPage;
      PageIndex := 0;
    end else if Compare(AData, pdPage^[pdCount-1]) > 0 then
