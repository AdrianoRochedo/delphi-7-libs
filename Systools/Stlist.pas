...
procedure TStList.conForEachPointer(Action : TIteratePointerFunc;
                                    OtherData : pointer);
var
  N : TStListNode;
  P : TStListNode;
...
{$ENDIF}
    ...
    if not Assigned(FHead) then begin
      ...
      while Assigned(P) do begin
        if Compare(N.Data, P.Data) < 0 then begin
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    if (Index < 0) or (Index >= FCount) then                   {!!.03}
      Result := nil
    else begin
      MinI := Index;
      MinP := FHead;
    ...
    repeat
      ...
      if L <> R then
        ...
        repeat
          ...
          repeat
            while Compare(PL.Data, PivotData) < 0 do begin
              PL := PL.FNext;
              Dec(Dist);
              Dec(DistR);
            end;
            while Compare(PivotData, PR.Data) < 0 do begin
              PR := PR.FPrev;
              Dec(Dist);
              Dec(DistL);
            end;
...
function TStList.conStoresPointers : boolean;
  begin
    Result := true;
  end;
