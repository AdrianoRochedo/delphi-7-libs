...
begin
  Result := True;
  if Container.Compare(Node.Data, SplitRec(OtherData^).SData) >= 0 then begin
    D := Node.Data;
...
procedure TStTree.conForEachPointer(Action : TIteratePointerFunc;
                                    OtherData : pointer);
var
  P : TStTreeNode;
  Q : TStTreeNode;
...
function TStTree.conStoresPointers : boolean;
  begin
    Result := true;
  end;
