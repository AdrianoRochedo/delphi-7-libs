...
begin
  if Code = ecOutOfMemory then
    ...
    E := ESTSortError.CreateResTP(Code);                       {!!.04}
    E.ErrorCode := Code;
    raise E;
  end;
end;
...
function MergeInfo(MaxHeap : LongInt; RecLen : Cardinal;
                   NumRecs : LongInt) : TMergeInfo;
type
  MergeFileSizeArray = array[1..(MaxBlockSize div SizeOf(LongInt))] of LongInt;
var
{$IFNDEF OS32}
...
{$ENDIF}
  ...
  Result.HeapUsed := AllocRecs*RecLen;
  ...
  if MFileCount > (MaxBlockSize div SizeOf(LongInt)) then
    Exit;
  ...
  while RecordsLeft > 0 do begin
    ...
    MFileSizeP^[MFileCount] := RecordsInFile*RecLen;
...
begin
  ...
  repeat
    ...
    {$ELSE}
    sorBuffer := GlobalAllocPtr(HeapAllocFlags, AllocRecs*FRecLen);
    {$ENDIF}
  ...
  sorMergeBufSize := FRecLen*(AllocRecs div (MergeOrder+1));
...
{$IFDEF OS32}
begin
  Result := PChar(sorBuffer)+Index*FRecLen;
end;
{$ELSE}
assembler;
asm
...
begin
  if sorMergeBytesUsed[M] >= sorMergeBytesLoaded[M] then begin
    ...
    if BytesRead < FRecLen then begin
...
begin
  sorCreateNewMergeFile(sorOutFile);
  try
    BytesToWrite := sorRunCount*FRecLen;
...
begin
  ...
  ArraySize := LongInt(RecLen)*NumRecs;
