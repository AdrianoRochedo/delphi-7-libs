...
{$IFDEF VirtualPascal}
  -- This unit is not compatible with OS/2 and Virtual Pascal
{$ENDIF}
...
{.Z-}
  public
    ...
    function GetFullKeyPath : string;                                  {!!.02}
...
var
  E : ESTRegIniError;
begin
  E := ESTRegIniError.CreateResTP(Code);                       {!!.04}
  E.ErrorCode := Code;
  raise E;
end;
...
var
  E : ESTRegIniError;
begin
  E := ESTRegIniError.CreateResFmtTP(Code, A);                 {!!.04}
  E.ErrorCode := Code;
  raise E;
end;
...
{$IFDEF OS32}
  BmpText.LoadFromResourceID(HInstance,stbmpText);
  BmpBinary.LoadFromResourceID(HInstance,stbmpBinary);
{$ELSE}
  BmpText.Handle := LoadBitmap(HInstance,MAKEINTRESOURCE(stbmpText));
  BmpBinary.Handle := LoadBitmap(HInstance,MAKEINTRESOURCE(stbmpBinary));
{$ENDIF}
...
function TStRegIni.WriteIniData(const ValueName : string; Data : String) : Boolean;
...
begin
  ...
  try
    GetMem(PValueName,Length(ValueName)+1);
    GetMem(PData,Length(Data)+1);

    strPCopy(PValueName,ValueName);
    strPCopy(PData,Data);

    Result := WritePrivateProfileString(riCurSubKey,PValueName,PData,riRootName)
  finally
    if PValueName <> nil then
      FreeMem(PValueName,strlen(PValueName)+1);
    if PData <> nil then
      FreeMem(PData,strlen(PData)+1);
  end;
end;
...
begin
  if ((Length(IString) div 2) <> Size) then begin
    Result := False;
    Exit;
  end;

  Result := True;
  for I := 1 to Size do begin                                          {!!.02}
    Index := (2*(I-1))+1;
    S[1] := '$';
    S[2] := IString[Index];
    S[3] := IString[Index+1];
...
procedure TStRegIni.ReadBinaryData(const ValueName : string; const Default; var Value; var Size : Integer);
...
{$ENDIF}
    if (riType = riIniType) then begin
      ...
      if (Len mod 2 = 0) then begin
        ...
        if not (StringToBytes(Values, PByte(Value), Size)) then        {!!.03}
...
{$IFDEF OS32}                                                          {!!.04}
  {$IFOPT H+}                                                          {!!.04}
        StrCopy(PValue, PAnsiChar(Value));                             {!!.04}
  {$ELSE}                                                              {!!.04}
        StrPCopy(PValue, Value);                                       {!!.04}
  {$ENDIF}                                                             {!!.04}
{$ELSE}                                                                {!!.04}
        StrPCopy(PValue, Value);                                       {!!.04}
{$ENDIF}                                                               {!!.04}
...
{$ENDIF}
    ...
    if (riType = riIniType) then begin
      ...
      HoldKey := 0;
      GetMem(PCSKey, Length(KeyName)+1 + strlen(riCurSubkey)+2);       {!!.01}
      GetMem(PSKey, Length(KeyName)+1);
      try
        PCSKey[0] := #0;
        StrPCopy(PSKey,KeyName);
        if riCurSubKey[0] <> #0 then                                   {!!.02}
          strcat(Strcopy(PCSKey, riCurSubKey), '\');                   {!!.01}
        strcat(PCSKey, PSKey);                                         {!!.01}
{$IFDEF OS32}
        ...
        ECode := RegCreateKeyEx(riPrimaryKey,PCSKey,0,nil,             {!!.01}
                 REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS,@FriSecAttr,
                 newKey,@Disposition);
        if (ECode <> ERROR_SUCCESS) then
          RaiseRegIniErrorFmt(stscCreateKeyFail,[ECode]);
...
{$ELSE}
        {creates a new key or opens an existing key}
        ECode := RegCreateKey(riPrimaryKey,PCSKey,newKey);             {!!.01}
        if (ECode <> ERROR_SUCCESS) then
          RaiseRegIniErrorFmt(stscCreateKeyFail,[ECode]);
...
{$ENDIF}
        ...
        FreeMem(PSKey,Length(KeyName)+1);
        FreeMem(PCSKey, Length(KeyName)+1 + strlen(riCurSubkey)+2);    {!!.01}
      end;
    end;
{$IFDEF ThreadSafe}
  finally
        ...
        with KeyInfo do begin
          ...
          if (SL.Count > 0) then begin
            for step := 0 to SL.Count-1 do begin
              ...
              P := pos('=',SL[step])-1;
              if (P > QIMaxVNLen) then
                QIMaxVNLen := P;

              P := Length(SL[step]) - P;
              if (P > QIMaxDataLen) then
                QIMaxDataLen := P;
            end;
          end;
        end;
...
function TStRegIni.KeyExists(KeyName : string) : Boolean;              {!!.01}
  {-checks if exists in INI file/Registry}
var
  KN : PAnsiChar;
