...
{$ENDIF}
    ...
    PTmp := '';
    while (posLCCh <= Length(Picture)) and (Upcase(Picture[posLCCh]) = Ch) do begin
      if S[posLCCh] <> ' ' then
...
{$ENDIF}
    ...
    K := J;
    while (J <= Length(Picture)) and (Upcase(Picture[J]) = UCh) do
      Inc(J);
    Dec(J);
  ...
  procedure MergePictureSt(Picture : string; var P : string;           {!!.02}
                          MC : ANSIChar; SP : string);                 {!!.02}
  var
    I, J : Cardinal;
    L    : Integer;                                                    {!!.02}
  begin
...
{$ENDIF}
    ...
    while Picture[I] = MC do begin
      {if J <= Length(SP) then}                                        {!!.02}
      if (L = 0) or (J > L) then                                       {!!.02}
        P[I] := ' '
      else begin
        P[I] := SP[J];
        Inc(J);
...
{$ENDIF}
    ...
    if OK then begin
      if (Length(t1159) = 0) and (Length(t2359) = 0) then begin
        ...
        I := 1;
        while (Result[TPos] = TimeOnly) and (I <= Length(P)) do begin
          Result[TPos] := P[I];
          Inc(I);
          Inc(TPos);
        end;
...
{$ENDIF}
    ...
    Len := Length(P);
    if OK then
      while (P[I+Result] = MC) AND ((I+Result) <= Len) do
        inc(Result);
  end;
  ...
  var
    ...
    begin
      ...
      if AL < DL then begin
        ...
        Temp := StrStDeleteS(Temp, I, AL-DL);
...
{$ENDIF}
  ...
  begin
    wTLZero := LongTimeFormat[2] = 'h';                        {!!.03}
    w12Hour := LongTimeFormat[length(LongTimeFormat)] = 'M';   {!!.03}

    wColonChar := TimeSeparator;
    wSlashChar := DateSeparator;

    {$IFDEF VirtualPascal}
    w1159 := GetProfileStr(SIntl, 's1159', 'am');
    w2359 := GetProfileStr(SIntl, 's2359', 'pm');
    {$ELSE}
