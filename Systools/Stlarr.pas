...
procedure TStLArray.Assign(Source: TPersistent);
  begin
    ...
    {$ENDIF}
      ...
      if not conAssignUntypedVars(Source, AssignArrayData) then
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    HugeFillChar(laData^, FCount*FElSize, 0);
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
  end;
...
procedure TStLArray.conForEachUntypedVar(Action : TIterateUntypedFunc;
                                         OtherData : pointer);
  var
...
procedure TStLArray.conGetArraySizes(var RowCount, ColCount, ElSize : Cardinal);
  begin
    RowCount := Count;
    ColCount := 1;
    ElSize := ElementSize;
...
procedure TStLArray.conSetArraySizes(RowCount, ColCount, ElSize : Cardinal);
  begin
    if (ColCount <> 1) then
      RaiseContainerError(stscTooManyCols);
    if (RowCount <> Count) or (ElSize <> ElementSize) then
      begin
...
function TStLArray.conStoresUntypedVars : boolean;
  begin
    Result := true;
  end;
...
{$ENDIF}
  ...
  HugeGetMem(laData, Elements*ElementSize);
  Clear;
end;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    if (not ElementsStorable) and (not Assigned(FStoreData)) then
...
procedure TStLMatrix.conForEachUntypedVar(Action : TIterateUntypedFunc;
                                          OtherData : pointer);
  var
    FullRow : ^TAssignRowData;
    i       : Cardinal;
...
procedure TStLMatrix.conGetArraySizes(var RowCount, ColCount, ElSize : Cardinal);
  begin
    RowCount := Rows;
    ColCount := Cols;
    ElSize := ElementSize;
...
procedure TStLMatrix.conSetArraySizes(RowCount, ColCount, ElSize : Cardinal);
  begin
    if (RowCount <> Rows) or (ColCount <> Cols) or
       (ElSize <> ElementSize) then
      begin
...
function TStLMatrix.conStoresUntypedVars : boolean;
  begin
    Result := true;
  end;
...
{$ENDIF}
  ...
  lmRowSize := LongInt(Cols)*ElementSize;
  {lmData := nil;}
  ...
  HugeGetMem(lmData, FCount*ElementSize);
  Clear;
end;
...
{$IFOPT R+}
    if Row >= Rows then
      RaiseContainerError(stscBadIndex);
{$ENDIF}
    move((PChar(lmData)+(Row*lmRowSize))^, RowValue, lmRowSize);
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
  end;
...
{$ENDIF}
    ...
    ProductOverflow(LongInt(Cols)*FRows, FElSize) then
      RaiseContainerError(stscBadSize);

    {compute and save various sizes}
    CurSize := FCount*FElSize;
    NewSize := LongInt(Cols)*FRows*FElSize;
...
{$ENDIF}
    ...
    ProductOverflow(LongInt(Rows)*FCols, FElSize) then
      RaiseContainerError(stscBadSize);

    CurSize := FCount*FElSize;
    NewSize := LongInt(Rows)*FCols*FElSize;
    CurFData := lmData;

    {allocate data block of new size}
    HugeGetMem(lmData, NewSize);

    FCount := LongInt(Rows)*FCols;
    FRows := Rows;
...
{$IFOPT R+}
    if Row >= Rows then
      RaiseContainerError(stscBadIndex);
{$ENDIF}
    move(RowValue, (PChar(lmData)+(Row*lmRowSize))^, lmRowSize);
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
  end;
    ...
    try
      with Reader do
        begin
          ...
          NumCols := ReadInteger;
          ElementSize := ReadInteger;
          if (NumRows <> Rows) or (NumCols <> Cols) or
             (ElementSize <> FElSize) then
            begin
              HugeFreeMem(lmData, FCount*FElSize);
              FElSize := ElementSize;
              FRows := NumRows;
              ...
              lmRowSize := LongInt(NumCols)*ElementSize;
              HugeGetMem(lmData, FCount*ElementSize);
              Clear;
            end;
          ElementsStorable := ReadBoolean;
