...
uses
  ...
  (*{$IFDEF NVBUG} LibIntf, {$ENDIF}*) {Rochedo}
  OvcCmd, OvcData, OvcMisc, OvcConst, OvcExcpt, OvcTimer, STDate;
(*
{$IFDEF NVBUG}
  {$IFDEF Win32}
  var
...
type
  TOvcController = class(TComponent)
  {.Z+}
  protected {private}
...
function FindController(Form : TForm) : TOvcController;
...
begin
  if not Assigned(AController) then begin
    ...
    if not Assigned(AController) then begin
      {$IFDEF NVBUG}
      (*CreateNVComponent(TOvcController.ClassName);*) {Rochedo}
