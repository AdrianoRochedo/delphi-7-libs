...
uses
  ...
  OvcEf, OvcExcpt, OvcMisc, OvcPb, OvcPf, STDate;
...
begin
  if not FDataLink.Editing then
    ...
    {$ELSE}
    DatabaseError(SNotEditing);   {Delphi 4}
    {$ENDIF}
...
{$ENDIF}
  ...
  try
    ...
    with FCanvas do begin
      ...
      {get text to display}
      if Field <> nil then begin
        if Field.IsNull then                                           {!!.01}
          S := ''                                                      {!!.01}
        else if FFieldType = ftDateTime then begin                     {!!.01}
          case FDateOrTime of
            ftUseDate : S := DateToStr(Field.AsDateTime);
            ftUseTime : S := TimeToStr(Field.AsDateTime);
          else
            S := Field.DisplayText
          end;
        end else
