...
begin
  ...
  try
    Bmp.Handle := LoadBitmap(HInstance, MakeIntResource(bmArrow));
    {$IFDEF Win32}
    ...
    Bmp.Handle := LoadBitmap(HInstance, MakeIntResource(bmEdit));
    clIndicators.AddMasked(Bmp, clWhite);
    Bmp.Handle := LoadBitmap(HInstance, MakeIntResource(bmInsert));
    clIndicators.AddMasked(Bmp, clWhite);
  finally
    Bmp.Free;
  end;
  ...
  try
    for I := 0 to Pred(clNumRows) do begin
      ...
      if Bool(IntersectRect(IR, CR, Clip)) then begin
        ...
        ExtTextOut(Canvas.Handle, Left, Y, {ETO_OPAQUE+} ETO_CLIPPED , {!!.11}
                   @CR, P, StrLen(P), nil);
