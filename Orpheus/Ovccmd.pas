...
{.Z-}
    ...
    procedure AddCommandRec(const TableName: string; const CR : TOvcCmdRec);
    ...
    function TranslateUsing(Tables : array of string; var Msg : TMessage) : Word;
    ...
    function TranslateKeyUsing(Tables : array of string; Key : Word; ShiftState : TShiftState) : Word;
...
begin
  ...
  else if CompareText(GetOrphStr(SCGridTableName), FTableName) = 0 then begin
...
procedure TOvcCommandProcessor.AddCommandRec(const TableName: string; const CR : TOvcCmdRec);
  {-add a command record to the command table}
var
...
procedure TOvcCommandProcessor.SetScanPriority(Names : array of string);
  {-reorder the list of tables based on this array}
var
  I      : Integer;
  Idx    : Integer;
...
function TOvcCommandProcessor.TranslateUsing(Tables : array of string; var Msg : TMessage) : Word;
  {-translate a message into a command using the given tables}
var
  TmpTbl  : TOvcCommandTable;
  Command : Word;
...
function TOvcCommandProcessor.TranslateKeyUsing(Tables : array of string; Key : Word; ShiftState : TShiftState) : Word;
  {-translate a Key and shift-state into a command using the given tables}
var
  TmpTbl  : TOvcCommandTable;
  Command : Word;
