...
begin
  ...
  if not efSaveData and FInitDateTime then
     begin
...
procedure TOvcCustomPictureField.efEdit(var Msg : TMessage; Cmd : Word);
  ...
  var
    ...
    begin
      ...
      if Succ(J-Delta) >= StrLen(efEditSt) then begin
        StrCat(efEditSt, ' ');
        if Delta > 1 then
...
ExitPoint:
    ...
    if Found then begin
      if (SaveHPos < DotPos) and (efHPos > DotPos) then begin
        pbFixNumber(DotPos, StBgn, StEnd, True, False);
        Inc(MF);
      end else if (SaveHPos > DotPos) and (efHPos < DotPos) then begin
