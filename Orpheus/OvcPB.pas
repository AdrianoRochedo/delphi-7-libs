...
begin
  ...
  if StrChPos(efPicture, pmCurrencyRt, I) then begin
    ...
    while (I+1 <= DestLen) and (efNthMaskChar(I) = pmCurrencyRt) do begin
      if J < K then begin
  ...
  if IsNum then begin
    ...
    if not FoundP then
      ...
      I := DotPosP+1;
    J := K+1;
    while (J+1 <= SrcLen) and (I+1 <= DestLen) and not pbIsLiteral(I) do begin
      Dest[I] := CopyOfSrc[J];
      Inc(I);
      Inc(J);
    end;
    ...
    {pad to end with 0's}
    while (I+1 <= DestLen) and not pbIsLiteral(I) do begin
      Dest[I] := '0';
      Inc(I);
    end;

    {handle trailing substitution characters}
    while (I+1 <= DestLen) and pbIsLiteral(I) do begin
...
begin
  ...
  SLen := 0;
  for I := 0 to MaxLength-1 do
    if (Found and (I = DotPos)) or not pbIsLiteral(I) then begin
      Inc(SLen);
      Dest[SLen-1] := Src[I];
    end;
  Dest[SLen] := #0;
