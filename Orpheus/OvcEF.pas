...
type
  ...
  protected {private}
    ...
    FCaretToEnd        : Boolean;      {caret is placed at end of string}
    ...
    FStripLiterals     : Boolean;      {remove literals from returned string}
    ...
    procedure SetSelText(const Value : string);                        {!!.13}
    ...
    function efGetTextExtent(S : PChar; Len : Integer) : Word;         {!!.10}
  ...
  protected
    ...
    procedure DoOnError(ErrorCode : Word; ErrorMsg : string);
      dynamic;
...
procedure TOvcBaseEntryField.DoOnError(ErrorCode : Word; ErrorMsg : string);
begin
...
begin
  ...
  if efHPos > StrLen(efEditSt) then
...
begin
  ...
  while (I < StrLen(P)) and (P[I] >= Space) do begin
    Inc(ChCnt);
    Inc(I);
  end;
  ...
  if (efFieldClass = fcNumeric) then                                   {!!.01}
    ...
    if efRightAlignActive then begin
      ...
      if X >= ClientWidth-TextMargin-1 then begin
        ...
        {the display string doesn't fit in the client area, so display it left aligned}
        X := TextMargin-1;
      end else
...
begin
  ...
  if Value < 0 then
      efHPos := 0
  else if Value > StrLen(efEditSt) then
...
begin
  ...
  if PopUpMenu = nil then begin
    ...
    try
      ...
      MI.Enabled := StrLen(efEditSt) > SelectionLength;
