...
type
  {event types}
  TDrawTabEvent =
    procedure(Sender : TObject; Index : Integer; const Title : string;
...
type
  ...
  protected {private}
    ...
    function GetPageHints(Index : Integer) : string;
    function GetPageName : string;
    function GetPageNames(Index : Integer) : string;
    ...
    procedure SetActivePageName(const Value : string);
    ...
    procedure SetPageHints(Index : Integer; const Value : string);
    procedure SetPageIndex(Value : Integer);
    procedure SetPageName(const Value : string);
    procedure SetPageNames(Index : Integer; const Value : string);
  ...
  protected
    ...
    procedure GetChildren(Proc : TGetChildProc
              {.$IFDEF VER100}; Root : TComponent {.$ENDIF});
      override;
    {$ENDIF}
    procedure Loaded;
      override;
  ...
  public
    ...
    function InsertPage(const Name : string;
    ...
    property PageHints[Index : Integer] : string
      read GetPageHints
      write SetPageHints;

    property PageNames[Index : Integer] : string
      read GetPageNames
      write SetPageNames;
...
function TOvcNotebook.GetActivePageName : string;
begin
  if nbIsValidIndex(PageIndex) then
...
function TOvcNotebook.GetPageHint : string;
begin
  Result := GetPageHints(PageIndex);
end;

function TOvcNotebook.GetPageHints(Index : Integer) : string;
begin
  if (Index > -1) and (Index < nbPageList.Count) then
...
function TOvcNotebook.GetPageName : string;
begin
  if nbIsValidIndex(PageIndex) then
    Result := TOvcNotebookPage(nbPageList[PageIndex]).Caption
  else
...
function TOvcNotebook.GetPageNames(Index : Integer) : string;
begin
  if (Index > -1) and (Index < nbPageList.Count) then
...
function TOvcNotebook.InsertPage(const Name : string;
                                 Index : Integer) : TOvcNotebookPage;
var
  Idx : Integer;                                                       {!!.14}
  {-insert a notebook page at the specified location}
    ...
    if (csDesigning in ComponentState) and (nbResizeWidth or nbResizeIndent) then
      ...
      for I := 0 to Pred(nbPageList.Count) do begin
        ...
        if PageIndex <> I then begin
          ...
          {is click in tab area and tab accessable}
          if (TP.PageEnabled or (csDesigning in ComponentState)) and
              PtInRect(TP.Area, Point(X,Y)) then begin
          ...
          if PtInRect(TP.Area, Point(X,Y)) then
            DoOnTabClick(I);
        end else if PtInRect(TP.Area, Point(X,Y)) then begin
...
begin
  ...
  if FTabOrientation = toTop then begin
    ...
    H := Self.Height-T-2;
    W := nbTabsPerRow*FTabWidth-5;
  end else begin
    T := 3;
    H := nbTabsPerRow*FTabWidth-5;
...
var
  ...
  procedure DrawTab(Index : Integer; Title : string; IsEnabled,
                    IsTabActive : Boolean;
  ...
  begin
    ...
    R := Rect(XPos, YPos, XPos+FTabWidth, YPos+TabH);
    ...
    with Canvas do begin
      ...
      R := Bounds(XPos+nbDefLeftMargin, YPos+TxtOfs, FTabWidth-nbDefLeftMargin*2, TabH-TxtOfs);
      ...
      if IsEnabled then begin
        ...
        if FShadowedText then begin
          ...
          DrawText(Canvas.Handle, @Buf, -1, R, DT_CENTER or DT_SINGLELINE);
...
procedure TOvcNotebook.SetActivePageName(const Value: string);
var
  I  : Integer;
begin
...
procedure TOvcNotebook.SetPageHint(const Value : string);
begin
  SetPageHints(PageIndex, Value);
end;

procedure TOvcNotebook.SetPageHints(Index : Integer; const Value : string);
var
  TP : TOvcNotebookPage;
begin
  if (Index > -1) and (Index < nbPageList.Count) then begin
...
procedure TOvcNotebook.SetPageName(const Value : string);
var
  TP : TOvcNotebookPage;
begin
  if nbIsValidIndex(PageIndex) then begin
...
procedure TOvcNotebook.SetPageNames(Index : Integer; const Value : string);
var
  TP : TOvcNotebookPage;
begin
  if (Index > -1) and (Index < nbPageList.Count) then begin
...
var
  ...
  Found   : Boolean;
begin
  {cycle through the string list adding new entries and copying}
  {data for existing entries in the page list as required}

  NewList := TList.Create;

  {delete any tab pages not in the new string list}
  ...
  {create new pages for any additional tab names in the string list}
