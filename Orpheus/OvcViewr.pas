...
begin
  ...
  with Params do
    Style := Style or ScrollBarStyles[FScrollBars]
...
function TOvcBaseViewer.GetMarkerPosition(N : Byte; var Col : integer) : LongInt;
  {-return the current position of the specified marker}
begin
  if (0 <= N) and (N < MaxMarkers) then begin
    Col := FMarkers[N].Col;
...
begin
  ...
  vwDitheredBG :=
     (Color <> GetNearestColor(Canvas.Handle, Color)) or
     (HighlightColors.BackColor <>
...
begin
  ...
  vwBMGlyphs.Handle := LoadBitMap(HInstance, 'MARKERS');
end;
...
begin
  ...
  if F.Handle = INVALID_HANDLE_VALUE then begin
    F.Mode := fmClosed;
    Result := GetLastError;
  end else begin
    F.Mode      := fmInput;
...
begin
  ...
  if soBackward in Options then begin
    ...
    while (not Found) and (fvWorkOffset >= LastOffset) do begin
      ...
      {$IFDEF Win32}
      if (Integer(fvWorkPtr) < fvWorkBeg) or (Cardinal(fvWorkPtr) >= fvWorkEnd) then
      {$ELSE}
      ...
      if (C = FirstCh) then begin
        ...
        if (PatternPos = 0) and (C = MatchString[PatternPos]) then
          ...
          for PatternPos := PatternPos+1 to PatternLen-2 do begin
            ...
            {$IFDEF Win32}
            inc(fvWorkPtr);
            if (Cardinal(fvWorkPtr) >= fvWorkEnd) then
              fvGetWorkingChar;
            {$ELSE}
            inc(OS(fvWorkPtr).O);
